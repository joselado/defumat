#!/usr/bin/env python3
"""Time defumat against Quantum ESPRESSO on a set of inputs and write a PDF.
...
**What it measures.** The project's primary metric: single-core defumat against
...
def measure(input_path: Path, repeats: int) -> dict:
    ...
    ours = compare_qe.run_defumat(input_path, repeats, conv_thr)
...
def environment() -> dict:
    ...
    from defumat import __version__ as defumat_version
...
def write_tex(results: list[dict], info: dict, path: Path,
    ...
    body = TEMPLATE.format(
        ...
        python=info["python"],
        jax=info["jax"],
        defumat=tex_escape(info["defumat"]),
...
\title{{defumat against Quantum ESPRESSO\\[2pt]
...
stopwatch around the process. The defumat SCF time is the \emph{{warm}} one:
...
defumat & {defumat}, commit \texttt{{{commit}}} \\
...
case & atoms & PWs & QE & defumat & ratio & QE & defumat & ratio & $\Delta E$ (Ry) \\
\midrule
{rows}
\bottomrule
\end{{tabular}}
...
\begin{{axis}}[
  ...
  xlabel={{defumat / QE, per SCF iteration}},
...
case & QE \texttt{{init\_run}} & defumat & cold & warm & QE & defumat & peak RSS (GB) \\
\midrule
{cold_rows}
\bottomrule
\end{{tabular}}
...
def main(argv=None) -> int:
    ...
    if args.from_json:
        ...
        print(f"defumat vs QE, {len(cases)} case(s), single core each")
