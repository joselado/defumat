...
| [`15_stress.ipynb`](15_stress.ipynb) | The stress tensor: `sigma = -(1/Omega) dE/d(epsilon)` from one `jax.grad` of the energy at frozen wavefunctions, on a *sheared* cell where every entry of the tensor is free, silicon's equation of state with `tr sigma/3` and `-dE/dV` plotted on top of each other, five references against `pw.x` (≤2.7e-7 Ry/bohr³), and the term-by-term breakdown that falls out of differentiating the terms instead of their sum | P11 |
