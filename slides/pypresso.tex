...
% ---------------------------------------------------------------- 9
\begin{frame}{Where the reference itself was wrong}
\begin{columns}[T]
\column{0.55\textwidth}
\small
\textbf{Validation cuts both ways.} Several findings here are about \qe, not
about pypresso:
\begin{itemize}\itemsep3pt
  \item \texttt{ph.x} 7.5's Raman branch does \textbf{not} reproduce QE's own
    committed example, and fails its \emph{own} internal check. Validation fell
    back to a finite difference of $\epsilon$ over re-converged displaced cells:
    \textbf{-3.118279} against \textbf{-3.118310}.
  \item \texttt{pw.x} reaches TB09 only through libxc, then passes a
    \textbf{zero Laplacian} and \textbf{never sets $c$} --- so what it runs
    under that name is Becke-Johnson.
  \item Committed benchmarks date from release 6.0 and have drifted;
    references are regenerated against the vendored build.
\end{itemize}
\column{0.42\textwidth}
\includegraphics[width=\textwidth]{figs/24_tran_blaha_band_gaps.png}
\\[2pt]
\footnotesize
\textbf{TB09 meta-GGA:} silicon's gap 0.49\,eV (LDA) $\rightarrow$
\textbf{1.13\,eV}, against experiment 1.17. Validated against \emph{analytic}
limits, not another code's floating point.
\end{columns}
\end{frame}
