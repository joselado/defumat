...
from defumat.io.pwin import PwInput, fortran_float, read_pw_input
from defumat.relax.settings import RelaxSettings
from defumat.system.cell import Cell, celldm_from_abc
...
class System(eqx.Module):
    ...
    #: (:mod:`defumat.workflows.anisotropy`), so its density comes from a
    ...
    #: A defumat extension with no pw.x counterpart -- Elk's ``bfcmt``: one
    ...
    #: :meth:`~defumat.calculator.Calculator.get_polarization`, so that a
    ...
    #: is no spiral. See :mod:`defumat.system.spiral`.
    ...
    #: :func:`~defumat.vdw.registry.canonical_vdw_corr`. ``'none'`` -- the
    ...
    #: runs on without one (:mod:`defumat.vdw`).
    ...
    #: as it is (:mod:`defumat.vdw.grimme`).
    ...
    #: reads (:class:`~defumat.relax.settings.RelaxSettings`). Static, and a
    ...
    #: defaults. Read by :func:`~defumat.workflows.relax.run_relax` and
    #: :func:`~defumat.workflows.vc_relax.run_vc_relax`.
    ...
    def with_spin(
        ...
        * **The weights.** ``setup.f90`` multiplies them by ``degspin`` only in
          ...
          somewhere else (:func:`~defumat.system.kpoints.for_spin`).
    ...
    def with_cell(self, at, positions=None) -> "System":
        ...
        *crystal* coordinates, which is what is fixed under a deformation.
        ...
        :meth:`~defumat.scf.driver.Calculation.at_cell`.
    ...
    def calculator(self, pseudos=None, **options):
        """A :class:`~defumat.calculator.Calculator` on this system.
        ...
        Imported inside the method: :mod:`defumat.calculator` imports this
    ...
    def symmetry_group(self, nosym: bool = False) -> Symmetries:
        ...
        **``nosym`` is the caller's decision, not this method's**, which is why
        ...
        ``nosym`` has no say over -- :func:`defumat.basis.builder` needs the
        fractional translations to size the FFT box, and
        :class:`~defumat.scf.driver.Calculation` keeps the full group beside a
...
def build_system(pwin: PwInput, precision: Precision = DEFAULT_PRECISION) -> System:
    ...
    # success. That is the same failure ``defumat.scf.continuation`` guards
    ...
    if any(float(v) != 0.0 for v in pwin.indexed("system", "b_field", 3)) and (
        ...
        raise ValueError(
            ...
            "refuses this and so does defumat; use one or the other"
        )
    ...
    # (:mod:`defumat.vdw.registry`) precisely so that an input asking for XDM
...
#: :mod:`defumat.vdw.registry` makes for the corrections it does not implement.
...
#: :meth:`defumat.scf.driver.Calculation.occupations`; the tetrahedron
#: spellings come from :data:`defumat.scf.tetrahedra.TETRAHEDRON_KINDS`, which
...
def _check_occupations(pwin: PwInput) -> None:
    ...
    :meth:`defumat.scf.driver.Calculation.occupations` dispatches ``fixed``,
    ...
    (:func:`defumat.scf.occupations.smearing_order` raises on an unknown one),
...
def _fsm_update(pwin) -> str:
    ...
    from defumat.scf.fields import DEFAULT_FSM_UPDATE, FSM_UPDATES
...
def _build_kpoints(
    ...
    if option in ("tpiba", "crystal"):
        ...
        # :func:`defumat.system.kpoints.expand_to_subgroup`.
        crystal_points = (
