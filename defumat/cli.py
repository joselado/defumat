...
``inspect`` prints what defumat reads out of a pw.x output, which is the
...
from defumat import __version__
from defumat.io import read_qe_output, write_dos
...
def _stress(args) -> int:
    ...
    from defumat.stress import compute_stress, format_stress
    ...
    # types ``defumat stress`` to be told that the input said no.
...
def _pdos(args) -> int:
    ...
    from defumat.io.output import write_pdos
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="defumat", description=__doc__.splitlines()[0])
    parser.add_argument("--version", action="version", version=f"defumat {__version__}")
