...
:func:`~defumat.stress.compute_stress` refuses ``method = 'analytic'`` by name.
...
from defumat.units import E2, FPI, TPI
from defumat.vdw.analytic import dispersion_stress
...
def _dvloc_of_g2(pseudo, q, omega):
    ...
    The same splitting :func:`~defumat.pseudo.formfactors.local_potential_of_g`
...
def _exchange_correlation(calculation, rho, gcart, factors, volume) -> dict:
    ...
    **``vtxc`` is against the valence density and ``etxc`` is over the total.**
    That asymmetry is the same one :func:`~defumat.scf.potential.exchange_correlation`
...
def _gradcorr(density_r, density_g, calculation, functional):
    ...
    :func:`~defumat.scf.potential.gradient_correction` makes for the potential.
    """
...
def _ewald(calculation, cell, gcart, volume):
    ...
    (:class:`~defumat.scf.ewald.EwaldSum`), not re-derived: the split is exact
