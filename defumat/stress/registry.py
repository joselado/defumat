...
The same two-implementation arrangement :mod:`defumat.forces.registry`
describes, one coordinate over:

``autodiff``
    ``-(1/Omega) grad`` of the total energy with respect to a strain applied to
    the cell (:mod:`defumat.stress.autodiff`). The default, because
...
``analytic``
    ...
    (:mod:`defumat.stress.analytic`). It is the reference the first one is
...
amount. :func:`~defumat.stress.compute_stress` therefore refuses
``method = 'analytic'`` by name and the terms are reached through
:func:`~defumat.stress.analytic_terms`, which is what the cross-check test
