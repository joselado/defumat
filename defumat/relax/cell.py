...
and with ``sigma = -(1/Omega) dE/d epsilon`` (:mod:`defumat.stress`) and the
...
inverse Hessian (:mod:`defumat.relax.bfgs`). The masks that also impose a
...
def cell_force(
    ...
    Args:
        stress: ``(3, 3)`` cartesian stress in Ry/bohr^3, with
            :mod:`defumat.stress`'s sign (a compressed crystal has a positive
            ``tr sigma / 3``).
        h: ``(3, 3)`` cell with the lattice vectors as **columns**, in bohr --
            the transpose of :attr:`defumat.system.cell.Cell.at`.
