...
of :mod:`defumat.xc.lda` return the local part and the routines here return the
...
``jax.grad`` of it in :mod:`defumat.xc.functional`. The identity that makes
that a drop-in replacement is ``v2 = 2 d(rho e)/d sigma``, since ``d sigma /
d|grad rho| = 2 |grad rho|``.

Units are Ry per bohr^3, following the convention of :mod:`defumat.xc.lda`:
