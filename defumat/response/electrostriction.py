...
:mod:`defumat.response.strain` (the ground state's own response to a strain)
...
carries an arbitrary constant (:mod:`defumat.response.strain`). It enters
...
(:mod:`defumat.response.strain`), but the *mixed* object being differentiated
...
``nspin = 1`` and insulators: everything :mod:`defumat.response.strain` and
...
from defumat.response.strain import (
    StrainResponse,
    mixed_becsum,
    strain_response,
    strain_tangent,
...
def _position_response(calculation, solver, rho, b, tangent, dpsi, drho,
    ...
    **The source term is written with the scalar and that is measured, not
    ...
    :meth:`~defumat.scf.driver.Calculation.at_strain` recorded.
    ...
    **And for an ultrasoft or PAW dataset the linear equation is not the whole
    ...
    (:func:`~defumat.response.efield.ultrasoft_position`, Dal Corso and Mauri
...
def susceptibility_strain_derivative(
    ...
    **The state tangent is ``dpsi + ort``** where the strain response carries an
    occupied block (:attr:`~defumat.response.strain.StrainResponse.ort`, and
    ``None`` for a norm-conserving dataset, where ``S`` does not deform). It is
    :func:`~defumat.response.nonlinear.susceptibility_displacement_derivative`'s
...
def require_converged_responses(field, strain) -> None:
    ...
    **That is why the loops no longer mix linearly**
    (:mod:`defumat.response.mixing`): the Anderson default takes this same cell
    ...
    for name, response, dial in (
        ...
        raise ValueError(
            ...
            "Anderson mixer (defumat.response.mixing), which converges cells "
    ...
    **response_options,
    ...
    Args:
        calculation: the :class:`~defumat.scf.driver.Calculation` the run used.
            ...
            :func:`~defumat.system.symmetry.symmetrize_cartesian_tensor` -- and
        ...
        response_options: passed to
            :func:`~defumat.response.efield.dielectric_tensor` and
            :func:`~defumat.response.strain.strain_response`. ``alpha_mix`` is
...
def _require_closed_for_elastic(calculation) -> None:
    ...
    (:func:`~defumat.system.symmetry.symmetrize_cartesian_tensor`, ``symmatrix3``
