...
def require_a_conductivity_regime(calculation) -> None:
    ...
    if calculation.is_ultrasoft or calculation.is_paw:
        raise NotImplementedError(
            ...
            "pseudopotential is not implemented, and three things are missing "
            "rather than one. The velocity here is matrix_elements, which is "
            "<n|dH/dk|m> and nothing else; a generalised eigenproblem's is "
            "<n|dH/dk - e_n dS/dk|m> plus the augmentation dipole "
            "<psi_n|T^dag dT/dk|psi_m>. Only the last of the three is written "
            "(defumat.topology.kubo.augmentation_connection, PLAN.md P98), and "
            "putting the other two in is not enough on its own: _resolvent_sum "
            "contracts z = <n|v_i|m><m|v_j|n> out of one `element` array and "
            "the two corrected factors are not transposes of each other -- the "
            "first takes K^dag and the second K -- so the sum needs a second "
            "array rather than a second index. The Drude term needs the "
            "e_n dS/dk piece too, since a band velocity carries it, and not "
            "the dipole, which vanishes on the diagonal. Use a norm-conserving "
            "dataset, where S = 1 and all three are identically zero, or "
            "defumat.topology.kubo for the static limit, which does carry them"
        )
