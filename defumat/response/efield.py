...
(:mod:`defumat.response.sternheimer`), with the commutator on the right.
...
which is exactly what :mod:`defumat.response.velocity` produces from one
...
that ``G = 0`` term (:func:`defumat.scf.potential.hartree`). The exchange-
...
force along the field's response, which is :mod:`defumat.response.born` and is
...
:meth:`~defumat.scf.driver.Calculation.symmetrize_directional`, which is
...
(:mod:`defumat.response.sternheimer`). The *position* operator is different: an
...
from defumat.scf.potential import hartree, total_charge
from defumat.system.symmetry import cartesian_rotations, symmetrize_matrix
...
class DielectricTensor:
    ...
    #: (:mod:`defumat.response.electrostriction`). Kept behind a flag because
...
def _require_a_finite_kernel(calculation, induced, density) -> None:
    ...
    **1504 of 91125 grid points have** ``|zeta| >= 1``, **and dv_of_drho has
    ...
    raise NotImplementedError(
        ...
        "zero and the product is inf * 0. Making defumat.xc's spin branch "
...
def _screening_kernel(calculation, density, screening: str):
    ...
    *and* ``f_xc``, which is what ``solve_e`` screens with and therefore what
    ...
    it exists for one reason: a sum-over-states response (:mod:`defumat.tddft`)
...
def ultrasoft_position(calculation, hamiltonians, states, position, dipole,
    ...
    (:func:`~defumat.response.electrostriction._position_response`). The two
...
def born_charges_zstar_eu(calculation, solver, v_scf, dpsi) -> np.ndarray:
    ...
    **This is not how the Born charges are computed here.** They come from
    ...
    (:mod:`defumat.response.born`), and this is the Fortran expression put
    ...
    ``zstar_eu_us.f90`` adds and :mod:`defumat.response.born` gets from one more
    tangent.

    The bare perturbation is ``dV_bare/du |psi>`` at frozen ``v_scf``, which is
    one ``jvp`` through :meth:`~defumat.scf.driver.Calculation.at_positions`
...
def require_a_symmetrisable_response(calculation) -> None:
    ...
    :meth:`~defumat.scf.driver.Calculation.symmetrize_directional`. With
