...
:mod:`defumat.response.phonon`'s construction with the *cell* in place of the
...
:mod:`defumat.response.strain`. The two are components of one tangent vector,
...
frozen while differentiating (:meth:`~defumat.scf.driver.Calculation.at_strain`),
...
:meth:`~defumat.scf.driver.Calculation.at_strain` does, so re-running the SCF on
...
class ElasticConstants:
    ...
    **Clamped-ion**, which for ``C_11`` and ``C_12`` in the diamond structure is
    ...
    :class:`~defumat.response.electrostriction.Electrostriction` are built from
...
def elastic_constants(
    ...
    **The density is rebuilt inside, and this is where a strain parts company
    with a displacement.** :func:`~defumat.response.phonon._force_constants`
    ...
    (:mod:`defumat.response.strain`). Letting the functional build its own
    ...
    **The price is that a symmetry-reduced k-set is refused**, since the
    ...
    the condition :mod:`defumat.response.electrostriction` imposes anyway.
    """
    if not allow_unconverged and response is not None and not response.converged:
        # The same refusal :func:`defumat.response.electrostriction.
...
def _require_a_closed_grid(calculation) -> None:
    ...
    if getattr(calculation, "_symmetry_maps", None) is not None:
        raise NotImplementedError(
            ...
            "defumat.response.electrostriction's own half runs on a wedge now, "
