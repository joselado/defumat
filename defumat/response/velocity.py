...
:meth:`~defumat.scf.driver.Calculation.at_kcart` is what rebuilds the two
...
term :mod:`defumat.forces.spiral` measures for ``dE/dq``.
...
class VelocityOperator:
    ...
    Built from the same three things :meth:`~defumat.scf.driver.Calculation.
    ...
    def __init__(self, calculation, v_scf, ddd_paw=None, ns=None, kcart=None):
        ...
        # calculation from :meth:`~defumat.scf.driver.Calculation.at_strain` it
...
def over_kpoints(hamiltonian, states, batch, overlap: bool = False):
    ...
    :mod:`defumat.solvers.davidson` walks the same axis.
    """
