...
:mod:`defumat.response.electrostriction` built P26's: **one ``jvp`` of the
...
response :mod:`defumat.response.phonon` solves for the dynamical matrix, ``b``
and ``u`` are the field response :mod:`defumat.response.efield` hands back
through ``keep_internals``, and ``db_c`` is the one further Sternheimer solve
P26 already wrote (:func:`~defumat.response.electrostriction._position_response`,
...
:func:`~defumat.system.symmetry.symmetrize_atom_cartesian_tensor`, applied to
...
(:func:`~defumat.response.electrostriction._second_order_energy_at`); getting it
...
class RamanTensors:
    ...
    #: The :class:`~defumat.response.phonon.DisplacementResponse` this was
...
def _epsilon_at(moved, psi, rho, b, u, weights, reference=None):
    ...
    :func:`~defumat.response.electrostriction._epsilon_at` with the geometry
...
def susceptibility_displacement_derivative(
    ...
    Args:
        ort: the occupied block of the first-order state
            (:func:`~defumat.response.phonon.orthogonality_states`). **Part of
        ...
        stored: the pre-tail electric-field solution,
            ...
            :func:`~defumat.response.electrostriction._position_response` so
        ...
        geometry_tangent: whether to carry the displacement's own tangent
            through :meth:`~defumat.scf.driver.Calculation.at_positions`, which
...
def require_a_complete_third_derivative() -> None:
    ...
    raise NotImplementedError(
        ...
        "defumat.response.nonlinear's module docstring. The Raman tensor, "
    ...
    **response_options,
    ...
    Args:
        ...
        result: the converged :class:`~defumat.scf.driver.SCFResult`. Its
            ...
            (:func:`~defumat.response.electrostriction.refined_states`), which
        ...
        keep_internals: hand back the displacement response on
            ...
            (:func:`~defumat.response.phonon.dynamical_matrix` takes it).
        allow_unconverged: return an answer even when a first-order response did
            not converge. Off, and for the reason
            :func:`~defumat.response.electrostriction.require_converged_responses`
