...
one ``jvp`` of ``H(k)`` (:mod:`defumat.response.velocity`, rule D2). The
...
- The first-order state ``|dn/dk>`` is what :mod:`defumat.response.sternheimer`
...
route). :mod:`defumat.response.velocity` records that a ``conv_thr`` of 1e-6
...
def effective_mass(
    ...
    Args:
        calculation: the :class:`~defumat.scf.driver.Calculation` the ground
            state was converged with -- its pseudopotentials and cell are what
            the stencil's NSCF runs are built from.
        result: the converged :class:`~defumat.scf.driver.SCFResult`. The
...
def _by_eigenvalue(calculation, result, centre, deltas, nbnd, conv_thr, k_batch):
    ...
    **The centre k-point is not in the difference, and that is not a detail.**
    ...
    This is the sphere-membership discontinuity :mod:`defumat.response.velocity`
...
def _stencil_kpoints(calculation, centre, offsets):
    """The stencil as a :class:`~defumat.system.kpoints.KPoints`.
