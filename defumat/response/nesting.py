...
* the mean of ``N`` over the whole ``q`` grid is exactly ``D(E_F)^2``, which
  ...
  in one number that :func:`defumat.workflows.dos.compute_dos` computes by a
...
def fermi_surface_weights(
    ...
    Args:
        ...
        smearing: a name from
            :data:`~defumat.scf.occupations.SMEARING_ORDER`.
    ...
    *,
    ...
    Args:
        eigenvalues: ``(nspin, nk, nbnd)`` or ``(nk, nbnd)`` in Ry, on the
            **complete** ``n1 n2 n3`` grid in ``monkhorst_pack`` order (last
            index fastest). :func:`~defumat.workflows.nesting.run_nesting`
    ...
    if g.shape[0] != nk:
        raise ValueError(
            ...
            f"first (defumat.workflows.nesting.run_nesting does it)"
        )
