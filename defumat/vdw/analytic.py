...
``jax.grad`` of :meth:`defumat.vdw.grimme.GrimmeD2.energy` in the two
coordinates, exactly as every other term's do. They are here for the reason the
transcribed Ewald, local and core forces are (:mod:`defumat.forces.analytic`) --
...
from defumat.vdw.grimme import _folded
...
def _pair_coefficients(dispersion, positions):
    ...
    :func:`defumat.vdw.grimme._dispersion_kernel`'s masking rule, verbatim:
