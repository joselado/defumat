...
:mod:`defumat.diffraction.structure_factor`; what is here is the part that is
...
:meth:`~defumat.scf.driver.Calculation.density` with masked weights, which
...
def run_structure_factors(
    ...
    *,
    ...
    Args:
        system: the run's :class:`~defumat.system.builder.System`.
        pseudos: its pseudopotentials.
        result: the :class:`~defumat.scf.driver.SCFResult` to transform. Only
...
def _core_density(system, pseudos):
    ...
    from defumat.pseudo.potentials import core_charge
