...
the stick layout and ``vkb`` do. :meth:`~defumat.scf.driver.Calculation.at_spiral_q`
...
contribution from the field or constraint machinery (:mod:`defumat.scf.fields`),
...
:mod:`defumat.system.spiral` documents and what an even Monkhorst-Pack grid
gives.
...
it is the ionic relaxation of :mod:`defumat.workflows.relax` with one
substitution: the coordinate is ``q`` instead of the atomic positions, the
gradient is :func:`~defumat.forces.spiral.compute_spiral_gradient` instead of
...
* **The surface has a floor of basis-set noise.** ``E(q)`` jumps by a
  ...
  (:mod:`defumat.forces.spiral`). Driving ``|dE/dq|`` below that scale is
...
#: :class:`~defumat.relax.bfgs.BFGSSettings`.
...
class SpiralScan:
    ...
    def integrated(self) -> np.ndarray:
        ...
        ``spiral_q`` and :attr:`~defumat.forces.spiral.SpiralGradient.gradient`
...
def run_spiral_scan(
    ...
    **scf_options,
    ...
    Args:
        ...
        keep_results: hold every :class:`~defumat.scf.driver.SCFResult`. Off by default: each one
...
def relax_spiral_q(
    ...
    **scf_options,
    ...
    The counterpart of :func:`~defumat.workflows.relax.run_relax` on the other
