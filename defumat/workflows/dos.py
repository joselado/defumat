...
* **smearing** (``PP/src/dosg.f90``) -- replace each level by a normalised
  ...
  construction.
* **tetrahedra** (:mod:`defumat.scf.tetrahedra`) -- interpolate the bands
...
:func:`defumat.scf.tetrahedra.tetrahedron_occupations_spin`).
"""
...
from defumat.workflows.nscf import denser_grid, run_nscf
...
def _tetrahedron_scheme(
    ...
    **_,
    ...
    """``tetra_dos_t`` / ``opt_tetra_dos_t``, via :mod:`defumat.scf.tetrahedra`."""
...
#: (:mod:`defumat.projwfc`). It is one implementation per family and not two,
...
def run_dos(
    ...
    (:class:`~defumat.calculator.Calculator` passes them for the caller).
