...
:func:`~defumat.topology.wilson.wannier_centers` already makes and it matters
...
def _refuse_ungapped(system: System) -> None:
    ...
    The same refusal every invariant in :mod:`defumat.topology` makes, and it
    is worth making at the workflow rather than letting the gap check inside
    :class:`~defumat.workflows.topology.DFTSource` report it per k-point: a
