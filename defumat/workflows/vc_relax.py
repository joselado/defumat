...
(:mod:`defumat.relax.bfgs`), minimising the **enthalpy** ``E + P Omega``. What
...
(:meth:`~defumat.scf.driver.Calculation.at_cell`). So the relaxation is one
...
(:mod:`defumat.relax.cell`), which is what ``press_conv_thr`` measures and why
...
class VCRelaxResult:
    ...
    #: :class:`~defumat.system.builder.System`, with its own k-points, so it
