...
from defumat.vdw.analytic import dispersion_force
...
def _terms(calculation, state) -> dict:
    ...
    if calculation.dispersion_sum is not None:
        # ``force_london``, transcribed in :mod:`defumat.vdw.analytic`. It is
...
def _xc_potential(calculation, rho):
    ...
    The same assembly :func:`~defumat.scf.potential.v_of_rho` does, minus the
...
def _force_ew(calculation, gcart, phases):
    ...
    :class:`~defumat.scf.ewald.EwaldSum` already fixed for the cell, neighbour
    list included.
    """
...
def _force_us(calculation, state, total, ddd_paw):
    ...
    :func:`~defumat.forces.energy.frozen_energy`.
    """
