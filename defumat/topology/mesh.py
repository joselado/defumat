...
Four shapes are needed and all four are the same object:

* a **plane mesh** closed in both directions -- what the Fukui-Hatsugai-Suzuki
  plaquette sum integrates over (:mod:`defumat.topology.berry`);
...
* a **string mesh**, open across the transverse plane and closed along one
  ...
  averaged over (:mod:`defumat.topology.polarization`);
* a **volume mesh**, closed in all three directions and neighboured both ways
  along each -- what a quantity built from a *derivative* rather than from a
  loop integrates over (:mod:`defumat.topology.orbital_magnetization`);
* the **time-reversal-invariant momenta**, a bare list of points
  (:mod:`defumat.topology.parity`).
...
class PlaneMesh:
    ...
    def kpoints(self, cell: Cell, precision: Precision = DEFAULT_PRECISION) -> KPoints:
        """The mesh as a :class:`~defumat.system.kpoints.KPoints` to diagonalise.
...
@dataclass(frozen=True)
class VolumeMesh:
    """A uniform grid over the whole zone, with both neighbours of every point.

    The plane and string meshes above are walked in one direction at a time and
    only ever need the *next* point; a quantity built from a k-derivative needs
    the point on either side, so this one answers
    :meth:`neighbour` for a sign as well as a direction.

    ``points`` is ``(n1, n2, n3, 3)`` in crystal coordinates, laid out so that
    the flat index runs with the **third** direction fastest -- which is QE's
    ``kpoint_grid_efield`` ordering (``n = (k-1) + (j-1) nk3 + (i-1) nk2 nk3 + 1``),
    so that a k-list generated here and one generated by ``pw.x`` for the same
    grid are the same list in the same order.

    ``shift`` is QE's ``k1, k2, k3``: a half-step offset per direction. It moves
    the mesh off the origin without changing anything about how it closes, since
    every neighbour is still a whole step away.
    """

    points: np.ndarray  # (n1, n2, n3, 3), crystal coordinates
    ...
    @property
    def nk(self) -> int:
        return int(np.prod(self.divisions))

    def flat(self) -> np.ndarray:
        """``(nk, 3)``: the points with the third direction fastest."""
        return self.points.reshape(-1, 3)

    def index(self, i, j, k) -> int:
        n1, n2, n3 = self.divisions
        return int((np.asarray(i) % n1) * n2 * n3
                   + (np.asarray(j) % n2) * n3 + (np.asarray(k) % n3))

    def neighbour(self, flat: int, direction: int, sign: int):
        """``(flat index, integer shift)`` of the point one step along ``direction``.

        ``sign`` is ``+1`` or ``-1``. The shift is the reciprocal lattice vector
        that has to be added to the returned point for it to be the physical
        neighbour -- nonzero only where the step leaves the mesh, which is the
        wrap the periodic gauge turns into a shift of the plane-wave index.
        """
        n = self.divisions
        cell = list(np.unravel_index(int(flat), n))
        moved = cell[direction] + int(sign)
        shift = np.zeros(3, dtype=int)
        if moved < 0:
            shift[direction] = -1
        elif moved >= n[direction]:
            shift[direction] = 1
        cell[direction] = moved % n[direction]
        return self.index(*cell), shift

    def kpoints(self, cell: Cell, precision: Precision = DEFAULT_PRECISION) -> KPoints:
        """The mesh as a :class:`~defumat.system.kpoints.KPoints` to diagonalise.

        Uniform weights, as :meth:`PlaneMesh.kpoints` uses them and for the same
        reason: nothing here integrates a density.
        """
        flat = self.flat()
        return KPoints.from_crystal(
            flat, np.full(len(flat), 1.0 / len(flat)), cell, precision=precision
...
def volume_mesh(
    ...
    """The whole uniform grid, in QE's ``kpoint_grid_efield`` layout.

    ``divisions`` is ``nk1, nk2, nk3`` and ``shift`` is ``k1, k2, k3``, so the
    point at cell ``(i, j, k)`` is ``((i + k1/2)/nk1, (j + k2/2)/nk2,
    (k + k3/2)/nk3)`` -- QE's expression, and no endpoint is repeated.
    """
    n1, n2, n3 = (int(d) for d in divisions)
    if min(n1, n2, n3) < 1:
        raise ValueError(f"a volume mesh needs at least one point per direction, got {divisions}")
    offsets = np.asarray(shift, dtype=float)
    points = np.zeros((n1, n2, n3, 3))
    points[..., 0] = ((np.arange(n1) + 0.5 * offsets[0]) / n1)[:, None, None]
    points[..., 1] = ((np.arange(n2) + 0.5 * offsets[1]) / n2)[None, :, None]
    points[..., 2] = ((np.arange(n3) + 0.5 * offsets[2]) / n3)[None, None, :]
    return VolumeMesh(points=points, divisions=(n1, n2, n3))
