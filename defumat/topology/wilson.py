...
2. **The pumping parameter runs over half the zone, ending exactly at 1/2** --
   ...
   Z2 quantity. See :func:`defumat.topology.mesh.pumping_mesh`.
3. **The charge-centre angles are the raw phases of the Wilson loop's
   eigenvalues** -- not routed through
   :func:`defumat.topology.links.berry_phase`, so they carry the opposite sign
...
:func:`defumat.topology.parity.fu_kane_z2` is for.
...
is written, **prefer :mod:`defumat.topology.parity` wherever the crystal has an
...
from defumat.topology.links import unitarize
