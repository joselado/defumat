...
Two implementations, behind the registry in :mod:`defumat.topology.registry`,
...
:mod:`defumat.topology.kubo`. What it once refused for ("``v = dH/dk`` needs
``d vkb/dk``, and the sphere gains and loses members with ``k``") is what P24
wrote: :class:`~defumat.response.velocity.VelocityOperator` is one ``jvp`` of
...
from defumat.topology.mesh import PlaneMesh
from defumat.topology.registry import register_curvature_method
...
def link_variables(states, mesh: PlaneMesh, k_batch="default"):
    ...
    :meth:`~defumat.topology.states.StateSet.overlaps` a direction at a time,
...
def fhs_curvature(states, mesh: PlaneMesh, k_batch="default") -> BerryCurvature:
    ...
    :func:`defumat.topology.links.berry_phase`. Both neighbour indices wrap,
...
def kubo_curvature(states, mesh: PlaneMesh, nocc: int | None = None, **_) -> BerryCurvature:
    ...
    Two routes, one expression. A :class:`~defumat.topology.states.ModelStates`
    carries ``H(k)`` as a differentiable dense matrix, so the whole thing is
    ``jacfwd`` plus an ``eigh``. A :class:`~defumat.topology.states.
    PlaneWaveStates` carries a :class:`~defumat.response.velocity.
    VelocityOperator` instead and nothing dense is formed --
    :mod:`defumat.topology.kubo`.
    ...
    if hamiltonian is None:
        from defumat.topology.kubo import plane_wave_kubo
        from defumat.topology.states import PlaneWaveStates

        if isinstance(states, PlaneWaveStates):
            # No dense ``H(k)`` to differentiate, and none is built:
            # :mod:`defumat.topology.kubo` contracts one ``jvp`` of the
...
def berry_curvature(
    ...
    from defumat.topology.registry import get_curvature_method
