...
(:class:`~defumat.hamiltonian.noncollinear.SpinorHamiltonian`) -- and, when the
...
def _starting_tau(rho, calculation) -> jnp.ndarray:
    ...
    both (:func:`defumat.scf.potential.with_core`), so the conversion would be
...
class SCFResult:
    ...
    becsum: tuple = ()
    #: The :class:`~defumat.system.builder.System` this was computed for. What
    ...
    #: The stress tensor (:class:`~defumat.stress.Stress`) when the run was
    ...
    #: the loop that moves the atoms (:mod:`defumat.workflows.relax`) rather
    #: than to one SCF's result.
    stress: object | None = None
    #: What a residual solver cost, when one was used
    #: (:class:`~defumat.scf.solvers.NewtonKrylovResult`). ``None`` for a
...
class Calculation:
    ...
    def __init__(
        ...
        # *density* has. See :class:`defumat.system.builder.System`.
        ...
        # (:mod:`defumat.vdw`). ``None`` when ``vdw_corr`` is 'none', which is
        ...
        # third decimal. See defumat.system.symmetry.
        ...
        # explicitly -- see defumat.paw.symmetry. Built after the symmetry
    ...
    def _require_meta_supported(self, system) -> None:
        ...
        * **Ultrasoft**, but not PAW. ``tau`` on the grid is the smooth
        ...
        (:func:`~defumat.scf.potential._noncollinear_meta_exchange`).
    ...
    def _spinor_overlap(self, psi: jnp.ndarray, ik: int) -> jnp.ndarray:
        ...
        :meth:`~defumat.hamiltonian.noncollinear.SpinorHamiltonian.apply_s`
    ...
    def at_positions(self, positions: jnp.ndarray) -> "Calculation":
        ...
        (:mod:`defumat.forces`). It is also what a relaxation step uses to move
        the atoms.
        ...
        * the **symmetry group**. QE finds it once and afterwards only *checks*
          ...
          the same relaxation. :func:`defumat.system.symmetry.check_symmetry`
    ...
    def at_cell(self, at: jnp.ndarray) -> "Calculation":
        ...
        *same* G-vectors against the new reciprocal cell and changes nothing
        ...
        # Used only by the analytic ``force_corr`` (:mod:`defumat.forces`), and
    ...
    def at_strain(self, strain: jnp.ndarray) -> "Calculation":
        ...
        :mod:`defumat.stress`). ``strain`` is a ``(3, 3)`` array; the
    ...
    def at_kpoints(self, kpoints) -> "Calculation":
        ...
        :mod:`defumat.topology` is built from states on a k-mesh or a loop, and
    ...
    def at_kcart(self, kcart) -> "Calculation":
        ...
        the velocity operator a ``jvp`` (:mod:`defumat.response.velocity`).
    ...
    def at_spiral_q(self, q_crystal, rebuild_basis: bool = True) -> "Calculation":
        ...
        (:mod:`defumat.forces.spiral`). Which plane waves are inside the cutoff
    ...
    def symmetrize_directional(self, fields: jnp.ndarray) -> jnp.ndarray:
        ...
        :func:`~defumat.system.symmetry.symmetrize_magnetization`'s
    ...
    def symmetrize_atom_tensor(self, tensors) -> np.ndarray:
        ...
        :mod:`defumat.response.efield` and :mod:`defumat.response.born`
        ...
        **symmetrised a ``nosym`` run** -- the group is kept whole beside
        ...
        for.
        """
        from defumat.system.symmetry import atom_mapping, symmetrize_atom_tensor
    ...
    def symmetrize_atom_pair_tensor(self, tensors) -> np.ndarray:
        ...
        :mod:`defumat.system.symmetry`). Guarded on :attr:`use_symmetry` for
    ...
    def symmetrize_cartesian_tensor(self, tensor) -> np.ndarray:
        ...
        """
        from defumat.system.symmetry import symmetrize_cartesian_tensor
    ...
    def symmetrize_atom_displacement(self, fields: jnp.ndarray) -> jnp.ndarray:
        ...
        (:func:`~defumat.system.symmetry.symmetrize_atom_displacement_density`).
    ...
    def symmetrize_strain_response(self, fields: jnp.ndarray) -> jnp.ndarray:
        ...
        (:func:`~defumat.system.symmetry.symmetrize_tensor_density`); the
...
def _result_for_stress(calculation, eigenvalues, wg, wavefunctions, rho, terms):
    ...
    :func:`~defumat.forces.energy.state_from_result` wants a finished
    ...
    :class:`~defumat.forces.energy.FrozenState` out of the loop's own arrays.
    """
...
def run_scf(
    ...
    (:mod:`defumat.scf.continuation`). Pass a
    :class:`~defumat.scf.continuation.ContinuedState` instead of the result
    ...
    input, through :attr:`~defumat.system.builder.System.tstress`, so that a
    ...
    :mod:`defumat.scf.solvers`). ``"mixing"`` is the loop below and the
    ...
    :mod:`defumat.scf.solvers` and ``PLAN.md`` P22.
    ...
    **A stress the input asked for and this code cannot produce is skipped with
    ...
    if starting_from is not None:
        ...
        # :mod:`defumat.scf.continuation` and there is no counterpart for the
        ...
        if starting_ns is None:
            ...
            if ns_state.shape != expected:
                raise ValueError(
                    ...
                    f"defumat.hubbard.uniform_ns / spin_averaged_ns, or start "
    ...
    if system.tstress if tstress is None else tstress:
        # Imported here rather than at the top of the module: ``defumat.stress``
