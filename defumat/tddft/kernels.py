...
dimensionless, ``(nw, nm, nm)``, on :class:`~defumat.tddft.chi0.ChiZero`'s
index set (three head directions then the body). Symmetrised because
:mod:`defumat.tddft.chi0` stores ``v^{1/2} chi_0 v^{1/2}`` and the two have to
...
def _alda(chi, epsi, context):
    ...
    :func:`~defumat.scf.potential.exchange_correlation` with a tangent of ones.
...
def alda_matrix(calculation, density, sphere) -> jnp.ndarray:
    ...
    """
    from defumat.scf.potential import exchange_correlation
    ...
    # box. This is :func:`~defumat.basis.fft.r_to_g` without its gather,
