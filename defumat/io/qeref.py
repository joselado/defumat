...
and through defumat and compare numbers, so this parser is the backbone of the
...
from defumat.units import ANGSTROM_TO_BOHR, RY_TO_KBAR
...
def comparison_table(rows, headers=("", "defumat", "reference", "difference"),
...
class QEReference:
    """Everything a defumat run can be checked against, from one QE output."""
    ...
    #: same names :mod:`defumat.stress.analytic` uses, **in Ry/bohr^3** -- QE
...
#: names :mod:`defumat.stress.analytic` gives the same contributions. Only the
