...
| **Berry curvature and Chern numbers**: exact integers on any mesh, and a smooth $\Omega(\mathbf{k})$ map with the truncation of its band sum reported. Norm-conserving, ultrasoft and PAW; a spinor run reaches the integers but not the map. | `run_berry_curvature`, `Calculator.get_berry_curvature`, `Calculator.get_chern`, `method="kubo"` for the map | | |
