...
| **Dielectric constant** $\epsilon^\infty$ and **Born effective charges** for insulators, on norm-conserving, ultrasoft and PAW, with the PAW $Z^{\ast}$ refused. Collinear spin including magnetic insulators, and a **spin-orbit** insulator carrying no net moment on norm-conserving; a *textured* spinor is refused by name. | `dielectric_tensor`, `Calculator.get_dielectric_tensor`, `Calculator.get_born_charges` | ✓ | ✓ |
