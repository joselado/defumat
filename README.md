...
| **DFT+U** — Dudarev's simplified functional with $U$, $J_0$, $\alpha$, $\beta$ and Liechtenstein's full one with $J$, $B$, $E_2$, $E_3$, selected by the card. Collinear or, for the simplified one, on a two-component **spinor** with spin-orbit coupling, where the occupation matrix is a 2x2 matrix in spin space. The intersite $V$ is refused by name | `HUBBARD` card, `noncolin`, `run_scf(starting_ns=...)` | ✓ | ✓ |
