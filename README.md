...
| **Dielectric constant** $\epsilon^\infty$ and **Born effective charges** — insulators, norm-conserving, ultrasoft and PAW (PAW $Z^{\ast}$ refused), **collinear spin** with magnetic insulators included, and a **spin-orbit** insulator carrying no net moment, norm-conserving (a *textured* spinor is refused by name) | `dielectric_tensor`, `Calculator.get_dielectric_tensor`, `Calculator.get_born_charges` | ✓ | ✓ |
...
| **Tunnelling spectra** $\mathrm{d}I/\mathrm{d}V(\mathbf{r}, V)$ — the same Tersoff-Hamann sum sectioned the other way: the curve at one place over many biases rather than one picture at one bias, which is what resolves a gap, a band edge or a state inside a gap. It is the measurement a **modulation** is actually read with, and the reason the row is here: a charge density wave is a gap that opens in antiphase with the charge maxima, a spin density wave moves the two channels in opposite directions from cell to cell, and none of that shows in an image at a single energy. The states are sampled at the tip points **once** and the energy axis is a matrix product after that, so 81 energies cost **41 times less** than 81 images; against the image route energy by energy it agrees to **2.5e-15** on a modulated ultracell. $I(V)$ comes back beside it as the integral, guarded against an axis too coarse to resolve the delta. A point, a line cut or a whole map per energy; a magnetic tip; unit cell or ultracell. Two things it refuses where an image does not, both because $\psi(\mathbf{r})$ is sampled and squared rather than summed into a density: a **symmetry-reduced** k-set, which nothing here symmetrises, and a tip inside an **augmentation sphere**. Neither code has an energy axis — `stm.f90` takes one scalar `sample_bias` and `wfplot.f90` sets one delta — so both are had by running the code again per bias, at the price of the whole sum each time | `run_sts`, `Calculator.get_sts`, `run_ultracell_sts`, `Calculator.get_ultracell_sts`, `energies`, `tip`, `height`, `spin`, `STMSpectrum.current` |  |  |
...
| **Magnons and the transverse spin susceptibility** $\chi^{+-}(\mathbf{q},\omega)$ — the collective precession of a magnet's own magnetization, and the Stoner continuum of independent spin flips it separates from. Its pole is the spin wave, and the Goldstone theorem pins it to zero energy at $\mathbf{q}=0$, which is the calculation's own error bar. Collinear magnets, insulating or metallic, norm-conserving **and ultrasoft** — the augmentation charge $Q_{ij}(\mathbf q+\mathbf G)$ inside every matrix element is what an ultrasoft magnet needs, and it is the same primitive the ultrasoft Berry phase already uses; PAW stays refused, for the one-centre part of its kernel rather than for the matrix element. $\mathbf{q}$ must be a difference of two k-points of the run's own grid, and the whole grid is needed rather than a wedge. Nickel's magnon agrees with Elk's committed reference to **0.8%** on the same cell and grid | `run_spin_susceptibility`, `Calculator.get_spin_susceptibility`, `run_magnon_dispersion`, `Calculator.get_magnon_dispersion`, `ecut_response`, `goldstone_correction` | (✓)¹² | ✓ |
| **Ultra long-range modulations** — a density or potential varying over tens or hundreds of unit cells, at a cost set by the number of *bands* rather than the number of plane waves. A spin density wave, a screened impurity or a domain wall is a slow envelope on a crystal that is still atomically periodic, and a supercell pays the same price for the envelope as for the atoms. Here the unit cell's own Kohn-Sham states at the $N$ folded k-points are computed **once** and used as the basis for an ultracell of $N = n_1n_2n_3$ cells, so the self-consistency runs on the envelope alone. It is a variational truncation of the exact $N$-cell supercell to `nbnd` bands per folded k-point and nothing else, and the induced density converges to a real supercell's as `nbnd` grows. Both magnetic regimes are in, so a **spin density wave** is reachable in either form: collinear spin modulates the moment's *length* under a scalar `magnetic_field`, with the two channels sharing one Fermi level, which is what lets an electron cross between them from cell to cell; a noncollinear run modulates its *direction* under a vector one, which is a helix or a cycloid and is a texture no collinear calculation can express. Spin-orbit coupling comes with it, since it lives entirely in the frozen states. Norm-conserving, LDA; the atoms do not move and the local band structure cannot relax | `run_ultracell`, `Calculator.get_ultracell`, `supercell`, `kgrid`, `nbnd`, `external`, `magnetic_field` |  | ✓ |
| **What a modulation looks like to a tip** — the Tersoff-Hamann image and the vertical tunnelling transmission of an *ultracell*, which is the observable a charge density wave, a spin density wave, a screened impurity or a domain wall is actually seen with. It costs nothing new: the union over $\mathbf{Q}$ of the $N$ unit-cell plane-wave spheres **is** the ultracell's own sphere at $\mathbf{k}_0$, so an ultracell state is a single coefficient vector on it and every routine written for a unit-cell wavefunction takes it unchanged — measured at **1.7e-16** against a peak of 0.14 for the state, and the image is then the density with a smeared delta at the tip energy in place of the occupations. A **magnetic tip** is what a spin density wave needs, since an unpolarized one does not see the wave but its **square**, two periods where the wave has one, at 37 per cent of its mean against the polarized tip's 82 on a cell carrying 0.12 Bohr magnetons. Against a real four-atom supercell under the same applied potential the error on the induced corrugation falls **22.2 → 5.2 → 2.0 per cent** over `nbnd` = 12, 24, 48, and widening the transmission's exit region to the whole ultracell gives the image back to **1e-11**, with no factor, which is what holds the two normalisations together. Elk plots an ultracell's density (tasks 731/2/3) and images a unit cell (task 162) and does not join them; QE has neither piece | `run_ultracell_stm`, `Calculator.get_ultracell_stm`, `run_ultracell_transport`, `Calculator.get_ultracell_transport`, `height`, `spin`, `exit_height`; the energy axis is `run_ultracell_sts` |  |  |
| **The energy of a long-range modulation** — whether a spin density wave, a charge density wave or a screened impurity is *worth* its own cost, which is the question an ultracell exists to ask and could not answer: what the run reported was the occupied eigenvalue sum, which double-counts the Hartree and exchange-correlation terms. This is the Kohn-Sham free energy per unit cell, assembled from the band sum the way an ordinary SCF's is, so the energy *gain* of a modulated state over the uniform one is a difference of two of these. It is also the one quantity in the method that converges with a **sign**: an eigenvalue and a density carry none, because the Hamiltonian moves with its own truncated density, while the bases are nested in `nbnd` and the energy is therefore a monotone upper bound on the real supercell's — measured at +8.15e-05, +3.52e-06, +3.62e-07 and +1.43e-07 Ry above a four-atom silicon supercell at `nbnd = 12, 24, 48, 64`, once both sides are put on the same FFT box. **Elk's ultracell has no total energy** (`energyulr.f90` is the eigenvalue sum alone) and `pw.x` has no ultracell | `run_ultracell`, `Calculator.get_ultracell`, then `UltracellResult.total_energy`, `.energy_terms`, `.energy_history`, `.field_energy` |  |  |
...
| **Relaxed magnetocrystalline anisotropy** — the same energy as the row above but from **total** energies rather than band sums: one full self-consistent noncollinear run per direction, with the density free to respond to the spin-orbit field instead of frozen. Costs an SCF per direction and buys the regimes the force theorem's density-only handoff cannot carry — **PAW** and a Hubbard $U$ — plus the relaxation energy itself. Reports how far each moment drifted from the direction it was started along, nothing holding it | `run_relaxed_anisotropy`, `run_relaxed_direction`, `Calculator.get_relaxed_anisotropy` | (✓)¹⁹ | ✓ |
| **Source-free exchange-correlation field, and the torque it exerts** — the longitudinal part of $\mathbf B_{xc}$ projected out so that $\nabla\cdot\mathbf B_{xc}=0$, which is the one thing that lets a local functional turn a moment at all: built pointwise from the length of $\mathbf m$, $\mathbf B_{xc}$ is parallel to $\mathbf m$ everywhere and the torque $\int \mathbf m\times\mathbf B_{xc}$ is **identically** zero at every density. It is a potential with no energy functional behind it, so the reported total is not the value of anything the run minimised and every derivative of it is refused; PAW, a collinear run and a spin spiral are refused by name | `nosource`, `Calculator.get_exchange_torque` | | ✓ |
