...
| **Berry curvature and Chern numbers**: exact integers on any mesh, and a smooth $\Omega(\mathbf{k})$ map with the truncation of its band sum reported. Norm-conserving, ultrasoft and PAW; the map is refused for an augmented *spinor* run, where the integers are still reachable. | `run_berry_curvature`, `Calculator.get_berry_curvature`, `Calculator.get_chern`, `method="kubo"` for the map | | |
