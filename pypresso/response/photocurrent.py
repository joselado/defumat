...
:meth:`~pypresso.response.velocity.VelocityOperator.matrix_elements`, one
``jvp`` of ``H(k)`` at a frozen sphere (P24), and ``w`` is
:meth:`~pypresso.response.velocity.VelocityOperator.second_matrix_elements`,
...
:mod:`pypresso.response.sternheimer` needs a positive definite one. It is P48's
...
from pypresso.scf.occupations import w0gauss
...
#: :mod:`pypresso.response.conductivity` follows. The difference is that a
...
class ShiftCurrent:
    ...
    Attributes:
        ...
        band_cut_gap: ``min_k (e_(nbnd+1) - e_nbnd)`` in Ry when the caller
            ...
            warning :class:`~pypresso.response.conductivity.
...
def shift_current(
    ...
    *,
    ...
    from pypresso.scf.occupations import smearing_order
...
def require_a_shift_current_regime(calculation) -> None:
    ...
    The first three are :func:`~pypresso.response.conductivity.
    ...
    if calculation.is_ultrasoft or calculation.is_paw:
        raise NotImplementedError(
            ...
            "pypresso.topology.kubo and pypresso.response.conductivity both "
    ...
    if _kpoints_are_reduced(calculation):
        raise NotImplementedError(
            ...
            "assembly does not do (pypresso.system.symmetry."
...
def _kpoints_are_reduced(calculation) -> bool:
    ...
    The same test :func:`~pypresso.response.conductivity._kpoints_are_reduced`
