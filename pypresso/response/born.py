...
(:func:`~pypresso.forces.energy.frozen_energy`), and the force is ``jax.grad`` of
...
still refused (:func:`~pypresso.response.phonon.require_norm_conserving`).
...
and ``becsum`` are handed to :func:`~pypresso.forces.energy.energy_at` as
builders rather than as arrays, for two reasons that pull the same way. One,
:meth:`~pypresso.scf.driver.Calculation.density` ends with the SCF's *scalar*
...
beside it (:func:`~pypresso.response.efield.born_charges_zstar_eu`) to
...
def require_born_charges(calculation) -> None:
    ...
    :func:`~pypresso.response.sternheimer.paw_response` already produces) with
...
def born_effective_charges(
    ...
    Args:
        calculation: the :class:`~pypresso.scf.driver.Calculation`, at the
            positions the states belong to.
        solver: the :class:`~pypresso.response.sternheimer.SternheimerSolver`
        ...
        projector_velocities: three ``d(vkb)/dk_a`` about each atom's own
            centre, which :mod:`pypresso.response.efield` has already built for
...
def _ground_state_multipliers(weights, eigenvalues, dtype):
    ...
    :func:`~pypresso.forces.energy.energy_at` use its diagonal form is what puts
...
def _raw_mixed_state(calculation, positions, psi, weights, density, becsum):
    ...
    :func:`~pypresso.forces.energy._mixed_state_part`'s convention. Each rebuilds
...
def _position_operator(calculation, projector_velocities):
    ...
    projectors per direction; :mod:`pypresso.response.efield` has already paid
    for it building ``adddvepsi_us``' position operator, so it is handed in
    rather than recomputed.
    """
    from pypresso.response.efield import _augmentation_dipole
...
def constraint_position_term(calculation, positions, solver, weights, commutator):
    ...
    **The one term here that is transcribed, and the reason is the same
    ...
    and :mod:`pypresso.response.efield` keeps it for the same reason.
    """
    natoms = positions.shape[0]
    if not calculation.is_ultrasoft:
