...
* **Two different offsets.** ``atomwfc_offset`` is where the manifold starts in
  ...
  occupation, exactly as :func:`pypresso.pseudo.atomic.atomic_channels` does,
...
class HubbardInput:
    ...
    **in Ry** -- the card writes eV and :mod:`pypresso.io.pwin` converts, so
...
def _atomic_orbitals(pseudo: Pseudopotential):
    ...
    :func:`pypresso.pseudo.atomic.atomic_channels` skips negative occupations,
