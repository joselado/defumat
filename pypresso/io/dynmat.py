...
:func:`~pypresso.response.spectra.mode_activities`, which is a transcription
...
* **the Raman block is indexed ``(i, j, displacement, atom)``** where this
  code's :attr:`~pypresso.response.nonlinear.RamanTensors.raman` is
...
from pypresso.units import AMU_TO_RY, BOHR_TO_ANGSTROM, FPI
...
def write_dynamical_matrix(
    ...
    title: str = "written by pypresso",
) -> Path:
    """Write a ``Gamma``-point ``fildyn`` that ``dynmat.x`` accepts.

    Args:
        cell: the :class:`~pypresso.system.cell.Cell`.
        structure: the :class:`~pypresso.system.structure.Structure`.
        force_constants: ``(nat, 3, nat, 3)`` in Ry/bohr^2 --
            :attr:`~pypresso.response.phonon.Phonons.matrix`. Written as the
        ...
        raman: ``(nat, 3, 3, 3)`` indexed ``[atom, displacement, i, j]``, in
            **inverse bohr** -- the raw
            :attr:`~pypresso.response.nonlinear.RamanTensors.raman`. Converted
    ...
    if raman is not None:
        ...
        # :attr:`~pypresso.response.nonlinear.RamanTensors.raman_angstrom2`
