...
:mod:`pypresso.topology.berry` has carried a ``kubo`` method since P16, and it
has only ever been reachable from a tight-binding
:class:`~pypresso.topology.states.ModelStates`, where ``H(k)`` is a small dense
...
both**: :class:`~pypresso.response.velocity.VelocityOperator` is one
