...
``"kubo"`` in :mod:`pypresso.topology.berry`. It is the wrong tool for an
...
``PlaneWaveStates``
    Kohn-Sham states from a pypresso run. Two things make it different from a
    ...
    * **The zone wraps.** On a closed mesh the neighbour of the last point is
    ...
    :mod:`pypresso.topology.augmentation`.
...
class StateSet(eqx.Module):
    ...
    def overlaps(self, pairs, k_batch: int | None | str = "default") -> jnp.ndarray:
        ...
        (:class:`PlaneWaveStates` through :func:`pypresso.batching.map_k`), and
...
def build_plane_wave_states(
    ...
    :meth:`~pypresso.scf.driver.Calculation.diagonalize` returns it for one spin
