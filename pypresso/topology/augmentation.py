...
:meth:`pypresso.hamiltonian.operator.Hamiltonian.apply_s` applies exactly that.
...
**Why this module rather than a call into** :mod:`pypresso.pseudo.augmentation`.
...
:meth:`pypresso.pseudo.spinorbit.SpinOrbitCoupling.qq_so` rather than writing the
...
``qq`` that :class:`~pypresso.pseudo.projectors.Projectors` already carries (and
...
from pypresso.pseudo.augmentation import (
    _assemble_qgm,
...
def augmentation_at_q(calculation, qcart) -> jnp.ndarray | None:
    ...
    Args:
        calculation: the :class:`~pypresso.scf.driver.Calculation` whose
