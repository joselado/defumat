...
**This is the Ewald sum's twin and is written as one** (:mod:`pypresso.scf.ewald`).
...
transcribed as *cross-checks* (:mod:`pypresso.forces.analytic`,
:mod:`pypresso.stress.analytic`) and not as the implementation.
...
:class:`~pypresso.scf.ewald.EwaldSum`'s choice and is why that one consults the
...
from pypresso.system.cell import Cell, fold_radius, lattice_translations
from pypresso.system.elements import atomic_number
from pypresso.system.structure import Structure
from pypresso.vdw.registry import register_vdw
...
def _dispersion_kernel(tau, c6, r_sum, translations, lattice, reciprocal, s6, rcut, beta):
    ...
    :func:`pypresso.scf.ewald._real_kernel`'s shape and its trap, verbatim: the
