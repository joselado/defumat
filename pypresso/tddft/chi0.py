...
and it is the one thing the rest of :mod:`pypresso.tddft` cannot get from code
that already exists: everywhere else in ``pypresso/response/`` the
...
:class:`~pypresso.response.velocity.VelocityOperator` -- one ``jvp`` of ``H(k)``
    ...
    *,
    ...
    Args:
        ...
        wavefunctions: ``(nspin, nk, nbnd, npwx)``, from a fixed-density run
            with empty states -- :func:`~pypresso.workflows.nscf.
