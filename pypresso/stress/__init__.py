...
:mod:`pypresso.stress.energy`; the sign is QE's, so that ``tr sigma / 3`` is the
...
* **The symmetry-forbidden components must vanish.** They do not, before
  :func:`~pypresso.system.symmetry.symmetrize_matrix`, because the
...
from pypresso.stress.analytic import ANALYTIC_TERMS, analytic_terms
from pypresso.stress.autodiff import autodiff_stress, autodiff_stress_terms
from pypresso.stress.energy import (
    require_a_differentiable_cell,
    strained_energy,
    strained_energy_terms,
)
from pypresso.stress.registry import (
...
def compute_stress(
    ...
    Args:
        ...
        method: ``'autodiff'`` (default). ``'analytic'`` is refused by name --
            see :mod:`pypresso.stress.registry` for why there is no analytic
    ...
    if name == "analytic":
        raise NotImplementedError(
            ...
            "pypresso.stress.analytic_terms for the term-by-term cross-check"
        )
