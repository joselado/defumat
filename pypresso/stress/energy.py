...
energy is written down once (:func:`pypresso.forces.energy.energy_at`) and the
derivative is ``jax.grad`` of it; what changes here is only which coordinate the
calculation is moved along -- :meth:`~pypresso.scf.driver.Calculation.at_strain`
instead of :meth:`~pypresso.scf.driver.Calculation.at_positions`. No expression
is derived for any contribution, and QE's ``PW/src/stress.f90`` and the eight
routines it calls are a *check* on this (:mod:`pypresso.stress.analytic`), not
the way it is done.
...
argument :mod:`pypresso.forces.energy` makes: at the converged solution the
...
the same trade :mod:`pypresso.forces.spiral` documents for ``q``, and a larger
...
reported total (:mod:`pypresso.scf.fields`), so the converged state is
