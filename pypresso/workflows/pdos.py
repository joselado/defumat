...
2. every band at every k-point is projected onto the Löwdin-orthogonalised
   pseudo-atomic orbitals of the crystal (:mod:`pypresso.projwfc.projections`);
...
is the *same* integration :mod:`pypresso.workflows.dos` performs, with a
...
:class:`~pypresso.scf.driver.SCFResult` the projection is of the states the SCF
...
:func:`pypresso.workflows.nscf.fixed_density_states`) -- and it costs nothing,
...
from pypresso.projwfc.channels import L_LABELS, M_LABELS, AtomicChannel, projection_channels
from pypresso.projwfc.projections import atomic_projections
from pypresso.pseudo.upf import Pseudopotential
from pypresso.scf.driver import Calculation
from pypresso.scf.tetrahedra import build_tetrahedra, tetrahedron_kind
...
from pypresso.workflows.nscf import NSCFResult, denser_grid, fixed_density_states
...
def compute_pdos(
    ...
    :func:`pypresso.projwfc.projections.atomic_projections` returns, which is
    ...
    :func:`pypresso.workflows.dos.compute_dos` does it.
    """
...
def run_pdos(
    ...
    ``result`` is the :class:`~pypresso.scf.driver.SCFResult` of that run -- the
