...
are :func:`pypresso.hubbard.projectors.build_atomic_projectors`'s -- the same
...
:mod:`pypresso.pseudo.spinorbit` already builds for ``fcoef``:
...
from pypresso.pseudo.spinorbit import LMAXX, rot_ylm
...
def orbital_matrices(l: int) -> np.ndarray:
    ...
    :func:`pypresso.pseudo.spinorbit.rot_ylm`, so the ordering of the ``m``
    index is ``ylmr2``'s -- ``m = 0`` first, then the cosine and sine of each
    ``|m|`` -- which is the ordering
    :func:`pypresso.projwfc.channels.projection_channels` labels.
    """
    if l < 0 or l > LMAXX:
...
def angular_momenta(
    ...
    Args:
        ...
        kind: the projector set, one of
            :data:`pypresso.projwfc.projections.PROJECTION_KINDS`. The default
    ...
    Returns:
        :class:`SiteAngularMomentum`.
    """
    from pypresso.hubbard.projectors import build_atomic_projectors
    from pypresso.projwfc.channels import projection_channels
    from pypresso.projwfc.projections import PROJECTION_KINDS
...
def _refuse_what_is_not_written(calculation) -> None:
    from pypresso.tddft.chi0 import _kpoints_are_reduced
