...
and a \emph{term} is missing rather than unchecked. With $\hat S = T^\dagger T$
the states a current is about are $T|\psi\rangle$, so the operator carries
$\langle\psi_n|T^\dagger\partial_k T|\psi_m\rangle$ --- the augmentation
dipole --- beside the $\epsilon_n\,\partial S/\partial k$ this assembly does
have, and there is nowhere in a formula written in $\partial H/\partial k$ and
$\partial S/\partial k$ to put it. Measured on a model where the exact answer
is free: \textbf{18\,\%} of a curvature and 0.010 of a Chern number. A \textbf{spin spiral}, whose two spinor
...
\begin{refuses}
\textbf{Refuses.} \textbf{Ultrasoft and PAW}, for the Kubo curvature's reason
one order further out. The dipole carries $\partial S/\partial k$ and its
derivative carries $\partial^2 S/\partial k_a\partial k_b$, and both are here;
what is not here is the augmentation dipole
$\langle\psi_n|T^\dagger\partial_k T|\psi_m\rangle$ that belongs with them, nor
its own derivative beside that. It is a missing term rather than an unchecked
convention, measured at 18\,\% of a curvature on a model. A
...
exact on any mesh. \textbf{Ultrasoft and PAW are refused by name}, and for a
missing term rather than an unchecked convention. The $\varepsilon_n\partial_k
S$ piece is right by derivation; what a moving $\hat S = T^\dagger T$ needs
beside it is $\langle\psi_n|T^\dagger\partial_k T|\psi_m\rangle$, the
augmentation dipole, and a formula written in $\partial H/\partial k$ and
$\partial S/\partial k$ has nowhere to put it. On a model built so the exact
curvature is known, leaving it out costs \textbf{18\,\%} of $\Omega$ and
\textbf{0.010} of the Chern number, and the two factors of the sum need
\emph{different} blocks --- so no rearrangement of $\partial S/\partial k$
supplies it. \code{method="fhs"} carries the whole thing as $q_{ij}(\mathbf b)$
on all three dataset kinds. The sum over empty states is
...
\code{ecutrho = 4 ecutwfc} is the only value an ultracell accepts, and it is
not what such a dataset wants: \code{run\_ultracell} says so with a warning,
because the run is then self-consistent and comparable against a supercell at
the same cutoffs while its absolute energy is not converged in \code{ecutrho}. Both datasets were measured against a real
...
\begin{refuses}
Ultrasoft and PAW datasets in a \textbf{noncollinear or spin-orbit} run: the
two scalar spin regimes are in, and what is missing at \code{npol = 2} is that a
spinor $D_{ij}$ is the scalar integrals sandwiched between \code{fcoef} rather
than the integrals themselves, so the displaced table would have to enter inside
that transform and the matrix element carry the $2\times2$ spin blocks with it.
...
The tunnelling \emph{transmission} of an ultracell on an augmented dataset
(\code{run\_ultracell\_transport}), whose exit-plane Gram matrix needs the
overlap operator $S$ --- where the image and the spectrum do run on one, because
a tip sits in the vacuum and sees the smooth states exactly, and both refuse a
tip inside an augmentation sphere as the unit cell's own versions do.
...
$\Gamma$. A double grid --- which is the wall an ultrasoft or PAW run meets
first whenever its input sets the \code{ecutrho} such a dataset wants: the
smooth half of the matrix element would then carry $\delta V$'s dense
components where \code{h\_psi} truncates them to the smooth sphere, and the
ultracell and the supercell it is validated against would not be discretising
the same functional. Set \code{ecutrho = 4 ecutwfc}, and read the warning that
comes with it --- \code{ecutrho} defaults to four times \code{ecutwfc} here as
it does in \code{pw.x}, so an input that simply leaves it out is \emph{accepted}
and runs with the augmentation charge on the wavefunction grid, which is the one
thing such a dataset exists not to do. Symmetry is not used at all: a modulation breaks the
