...
left refused is the \emph{strain} leg alone. Beyond that: insulators, \code{nspin = 1}, and
...
\begin{refuses}
\textbf{Refuses.} \textbf{Ultrasoft and PAW}, and three things are missing
rather than one. The velocity here is $\langle n|\partial_k H|m\rangle$ and
nothing else, where a generalised eigenproblem's is
$\langle n|\partial_k H - \varepsilon_n \partial_k S|m\rangle$ plus the
augmentation dipole $\langle\psi_n|T^\dagger\partial_k T|\psi_m\rangle$. Only
the last of the three is written, as
\code{topology.kubo.augmentation\_connection}, and putting the other two in is
not enough on its own: this module contracts
$z = \langle n|v_i|m\rangle\langle m|v_j|n\rangle$ out of one array and the two
corrected factors are not transposes of each other, the first taking
$K^\dagger$ and the second $K$, so the sum needs a second array. The Drude term
needs the $\varepsilon_n\partial_k S$ piece as well, since a band velocity
carries it, and not the dipole, which vanishes on the diagonal. For the static
limit alone, \code{topology.kubo} carries all three. A
\textbf{spin spiral}, whose two spinor
...
one order further out, and one of the four terms it is missing is now written.
The velocity and its derivative here are $\langle n|\partial_k H|m\rangle$ and
$\langle n|\partial^2_k H|m\rangle$ and nothing else, where a generalised
eigenproblem's carry $\varepsilon_n\partial_k S$ and
$\varepsilon_n\partial^2_k S$ beside them, and the connection carries the
augmentation dipole $\langle\psi_n|T^\dagger\partial_k T|\psi_m\rangle$ and,
one order further out, $\partial_k(T^\dagger\partial_k T)$. Only the third is
written, as \code{topology.kubo.augmentation\_connection}; the fourth is a
second projector derivative against $\mathrm{dpqq}$ and $\mathrm{dpqq}$'s own
first moment. A
...
\begin{equation*}
  ...
  + (\varepsilon_n - \varepsilon_m)\, L^{a}_{nm}
\end{equation*}
...
The last term is what an \textbf{ultrasoft or PAW} dataset adds, and it is zero
for a norm-conserving one. With $\hat S = T^\dagger T$ the states a Berry phase
is about are $T|\psi\rangle$, so the connection carries
$K^{a}_{nm} = \langle\psi_n|T^\dagger \partial_{k_a} T|\psi_m\rangle$ beside
$\langle\psi_n|\hat S\,\partial_{k_a}\psi_m\rangle$, and that piece is a
function of $T$ rather than of $\hat S$: replacing $T$ by $U(\mathbf k)T$
leaves $\hat S$ alone and moves the physical states, so no arrangement of
$\partial S/\partial k$ can supply it. The two factors take \emph{different}
blocks --- $L = K^\dagger$ in the first and $K$ in the second, with
$L + K = \partial S/\partial k$ exactly --- and
\code{augmentation\_connection} builds $K$ from the projectors' derivative
about the atom's own centre and $\mathrm{dpqq}$, the augmentation charge's
dipole.
...
The augmented term has its own anchor, and it is an identity rather than a
reference value, because nothing in a norm-conserving run can see a term that
vanishes there. The same $K^{a}$ is the $\mathbf k$-derivative of the
\emph{two-point} overlap $S(\mathbf k, \mathbf k')$ at frozen coefficients ---
Fukui-Hatsugai-Suzuki's own matrix, which shares no eigenvalue, gap or
\code{jvp} with the sum --- and a central difference of it reproduces
\code{augmentation\_connection} to $8.5\times10^{-10}$ against a
$\max|K| = 2.1\times10^{-2}$, falling by a clean factor of four per halving of
the step. Independently, $K + K^\dagger$ is $\partial S/\partial k$ to
$1.4\times10^{-16}$. The two checks are both kept because the second cannot see
the dipole at all: $\mathrm{dpqq}$ enters as $-iD$ with $D$ Hermitian, so it
cancels out of $K + K^\dagger$ with any sign and any size. On ultrasoft AlAs at
a generic $k$-point, with the band sum converged at \code{nbnd = 130}, the term
is worth \textbf{0.57\,\%} of $\Omega$ and the whole of what an augmented
overlap does is worth 2.34\,\%. Ultrasoft and PAW silicon still vanish
pointwise, at $3.9\times10^{-5}$ and $1.4\times10^{-4}$.
...
exact on any mesh. A \textbf{spinor} ultrasoft or PAW run is refused by name,
and by a term: its overlap carries $q^{\mathrm{so}}_{ij}$, the \code{fcoef}
transform of $q_{ij}$ into a $2\times2$ matrix in spin space, and the
connection's dipole is that same transform applied to $\mathrm{dpqq}$, which is
not written. A \emph{scalar} ultrasoft or PAW run is supported and tested; that
refusal is gone. \code{method="fhs"} carries the whole thing as
$q_{ij}(\mathbf b)$ at every \code{npol} and remains the route an invariant is
read from. The sum over empty states is
...
is the true one. An ultracell is exempt from
both: it requires \code{nosym} and refuses ultrasoft and PAW already.
