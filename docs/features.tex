...
$=0$); a PAW dataset, whose one-centre field turns the moments too and is not in
the torque; a spin spiral, whose transverse magnetization is kept in the frame
that turns with $\mathbf q$; a Hubbard $U$, whose \code{ns} would have to turn
with the density; a magnetic field or a constrained moment, which holds the
orientation the step moves; and a run without \code{nosym}.
\code{get\_relaxed\_orientation} and \code{get\_orientation\_torque} refuse
everything the force theorem refuses, and a PAW torque, whose one-centre
coefficients are not yet rebuilt from the turned \code{becsum}.
\end{refuses}
