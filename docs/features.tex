...
\textbf{A constraint does not rescue it, and that edge is worth knowing.} A
spiral SCF accepts \code{constrained\_magnetization} and a magnetic field ---
...
\code{'fsm'} does not hold one: at $q_3 = \tfrac12$ with the target at the same
cell's $q = 0$ moment the run takes 200 iterations without converging and
overshoots it by a factor of five. \code{'fsm'} is a \emph{feedback} field rather
than a penalty, so \code{constraint\_energy} is 0 and nothing on the result says
how far from its target the run is --- the failure reads as an ordinary
non-convergence. Elk's own magnon-spiral example runs \code{fsmtype = -1} with a
large field for exactly that reason: the \emph{direction} fixed hard, not a small
magnitude pinned. That scheme is not implemented here.
