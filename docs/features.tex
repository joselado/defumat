...
print(abs(cheap.e14 - piezo.e14))   # 6.2e-15
\end{pycode}
...
extends to the next derivative; reach for the other, **on a norm-conserving
dataset**, when the mesh is what you are converging. On an ultrasoft or PAW one
it is refused by name, because \code{zstar\_eu.f90} hands that case to
\code{zstar\_eu\_us.f90} and the transcription stops at the first file: measured
on ultrasoft AlAs at 64 $k$-points it reads $+0.830702$ against the
differentiated route's $+0.815802$, 1.8 per cent apart.
...
rather than on the total energy}, and note that nothing in the code warns when
you have not.
...
out, so it measures the $k$-convergence above rather than the dataset. Beyond that: insulators, \code{nspin = 1}, and
