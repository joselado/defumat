...
with the converged energies identical. The saving is the whole array minus the
core, so it scales with \textbf{atoms per species}: nearly all of it on 45 atoms
of two species, and little on a cell with one atom per species. At a single
$k$-point it is a small \emph{loss}, because there is nothing to save and the
rebuilt chunk is the whole array. It costs $n_{\rm at} n_{\rm pwx}$ complex
exponentials each time a $k$-point's projectors are used, so it is a memory dial
and not a speed one. The default is \code{store}.
