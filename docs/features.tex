...
Every \textbf{response} quantity --- the dielectric tensor, phonons, Raman,
Born charges --- is refused by name under gamma storage, and the reason is that
it does not fail: the Sternheimer stack's inner products carry no
$2\,\mathrm{Re}(\sum) - (G{=}0)$, and silicon's dielectric constant comes out
$285.4/229.4/228.3$ against an isotropic $190.8$ --- not even cubic on a cubic
crystal, which is the only tell. The partial dynamical matrix and gamma storage
therefore do not combine yet. A checkpoint refuses rather than drops a converged
