...
space group is written, so a spiral needs \code{nosym}; $dE/d\mathbf q$, and so
\code{relax\_spiral\_q} and the integrated route through \code{gradients=True},
on an \textbf{ultrasoft or PAW} dataset --- the \emph{ground state} of such a
spiral runs, but the displaced table above is itself a function of $\mathbf q$
and a gradient taken at a frozen table would be wrong by the whole
$dQ_{ij}(\mathbf G - \mathbf q)/d\mathbf q$ term, which is refused by name
rather than differentiated anyway; and spiral relaxation under a magnetic field,
whose energy is outside the reported total, so the state is stationary for a
