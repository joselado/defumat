...
moment from 0.027 to 0.719 --- and a secant iteration on a step overshoots by
construction. Past the overshoot the moment is saturated and the inner SCF stops
converging under the large field, and since the field steps only on
self-consistent pairs it then freezes where it went. Use a cell whose $m(B)$ is
not a step, and give \code{electron\_maxstep} room: it is \textbf{shared} between
the inner SCF and the outer field loop.
