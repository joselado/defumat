...
The spiral's spin rotation is about $z$, so a magnetization along $z$ is
invariant under it: a cone of zero opening angle, and a stationary point at
every $\mathbf q$. Carried into a spiral run it converges, reports a moment,
and has computed the ferromagnet. Give the target an \code{angle1} away from
zero, or pass \code{magnetization='seed'}.
