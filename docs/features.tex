...
conjugation; \code{nosym = .true.} is the way out), and for a spinor or spiral
run. \code{estimate()} makes the same decision the driver makes, so it sizes
the run either way.
...
Validated on a fully-relativistic \textbf{PAW} platinum dataset against a
\code{projwfc.x} run generated for it, three ways. The \emph{columns} --- how
many, and each one's $(l, j, m_j)$ --- match QE's own label table exactly, 18
channels where the $j$-averaged count is 9. The \emph{projections} agree to
...
And the \textbf{Löwdin charges and the spilling parameter agree to every digit
\code{projwfc.x} prints}. Timing, one core each from a converged ground state:
...
Ultrasoft is the one kind not claimed here: a fully-relativistic ultrasoft
dataset whose \code{PP\_QIJ} is stored without its $L$ decomposition has a
separate, open defect in its ground state, so there is no converged state to
project. Norm-conserving is untested rather than refused --- no reference case
is committed for it.

\begin{refuses}
\textbf{Refuses.} A \textbf{symmetrised} spinor projection: averaging over the
point group needs the SU(2) representation of each operation beside the rotation
of the harmonics, and that is not built --- run with \code{nosym} and the whole
$\mathbf k$-grid, where both codes use every point and neither averages.
\textbf{\code{noncolin} without \code{lspinorb}}: there is no $j$ to resolve by,
and the up/down projection that case wants is not built here. An
$m$-resolved charge is \emph{absent} rather than refused: \code{charges\_lm} is
\code{None} for a spinor run, since a spin-angle function has an $m_j$ and no
$m$, and there is no $p_x$ weight to report.
\end{refuses}
...
phonons at $q\neq0$; a partial dynamical matrix (\code{atoms=}) of a
...
across the moment takes exactly half. The same cell run as $n_{\rm spin}=1$,
...
nothing extra. A spin-selective substrate on a run with no magnetization, and a
transverse direction on a collinear one. And, inherited from the STM images
