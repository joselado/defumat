...
Meta-GGA. \textbf{Noncollinear} magnetism and spin-orbit coupling, for the shape
of the matrix rather than the physics: a collinear ultracell matrix is block
diagonal in spin and so is two of the same build, where a spinor state is one
vector of $2\,\code{npwx}$ components and the potential acts on it as
$V_0 + \boldsymbol\sigma\cdot\mathbf B$. A ground state converged \textbf{under
