...
mixing at $0.85$ under an adaptive name. Leaving \code{mixing\_beta} unset is
therefore the right thing --- each mode then supplies its own default, $0.7$ for
the QE family and $0.05$ here --- and passing a step length warns rather than
running quietly. On the two-atom silicon cell of \code{benchmarks/si-1k.in} at
\code{conv\_thr = 1e-10} it takes 17 iterations against Anderson's 7, to the same
total energy to ten digits: it is a \emph{robustness} mixer and is expected to
lose on a cell that was never hard.

\begin{refuses}
\code{mixing\_mode = 'adaptive'} refuses a \textbf{preconditioner} rather than
ignoring one. Its step is pointwise in real space with its own factor per point,
