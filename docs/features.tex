...
\noindent or \code{python3 -m defumat.cli size scf.in}. It reports $n_{gm}$,
...
twice the storage --- and it says so; \code{estimate()} sizes the substitution
rather than the request, since reporting the halved counts would understate
every array by two. A checkpoint refuses rather than drops a converged
