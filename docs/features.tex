...
basis does not have. \code{kramers\_pairs=True} adds each frozen state's Kramers
partner, the states of the reference with its magnetization reversed, from a
second fixed-density solve, and diagonalises the reference Hamiltonian in the
union once, before the loop. Without spin-orbit coupling that span is the
...
the matrix. With spin-orbit coupling the partners remove only the preference
...
# the same helix on a basis closed under time reversal, a third of the size
...
\begin{refuses}
\code{kramers\_pairs} on a collinear cell, whose one channel's partner is the
other channel and is already in the basis.
