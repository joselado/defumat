...
extrapolating mixer is free to walk along it. The weaker the applied field, the
less it pins that direction and the further the walk: on the four-cell hydrogen
ultracell above, a $0.002$~Ry turning field at \code{mixing\_beta = 0.7} reaches
a per-cell moment of $2.2\,\mu_B/2$ on an atom holding \emph{one} electron,
which is a state that cannot exist. The same \code{beta} at $0.01$~Ry converges.
Lowering it is the answer, and the run names the mechanism when it fails rather
