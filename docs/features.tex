...
\code{get\_orientation\_torque} & the torque on a whole texture, three components \\
\code{get\_relaxed\_orientation} & the easy orientation of a texture, by BFGS in the rotation \\
...
\code{mixing\_beta\_mag} gives the \textbf{magnetization} a step length of its
own, separate from the charge's \code{mixing\_beta}. It is this code's own knob
and a deliberate departure from \code{pw.x}, which uses one \code{alphamix} for
every component of its density; VASP exposes the same thing as \code{AMIX\_MAG}
and Elk gives the magnetic channel its own control. Unset by default, which is
QE's rule, and setting it equal to \code{mixing\_beta} reproduces an unset run
exactly rather than nearly.

The reason it exists is that the two channels are slow for different reasons and
one scalar cannot serve both. The charge's bad direction is the long-wavelength
Hartree one, which Kerker and \code{local-TF} compress; the magnetization's is
the Stoner enhancement $\chi_0/(1 - I\chi_0)$, which is local, has no $1/q^2$
for a preconditioner to divide out, and amplifies a uniform change at every
wavelength equally. Nothing conditions that direction, so a step length is all
there is.

\textbf{Whether to use it is a measurement rather than a preference, and the
measurement is \code{residual\_split}.} \code{run\_scf(residual\_split=True)}
puts a five-way split of each iteration's residual into
\code{SCFResult.history} --- charge, the length of $\mathbf m$, the rigid
rotation, the rest of the transverse channel, and the magnetic part of
\code{becsum} --- and it is what says which channel a given cell is spending its
iterations in. The two committed iron benchmarks are opposite cases and the knob
follows the split, not the cell:

\begin{center}
\begin{tabular}{lrr}
\toprule
\code{mixing\_beta\_mag} & \code{fe-noncolin-pbe-stress} & \code{fe-mag-1k} \\
\midrule
unset & 43 & 25 \\
equal to \code{mixing\_beta} & 43 & 25 \\
$0.6$ & \textbf{27} & 36 \\
$1.0$ & \textbf{20} & 31 \\
\bottomrule
\end{tabular}
...
The first cell spends 28 of its 43 iterations in the magnetic channel and the
knob takes it to 20, against \code{pw.x}'s 19 on the same input. The second
spends 4 of 25 there, and the same knob makes it a third \emph{worse}. Every run
in that table converged to the same total energy as its own unset baseline within
$3\times 10^{-10}$~Ry, so these are the same states reached sooner or later and
not different ones.

\begin{refuses}
\code{mixing\_beta\_mag} is \textbf{refused at setup} with
\code{mixing\_mode = 'adaptive'} rather than ignored. That scheme already
carries one step length per component of the mixed vector, so a second global one
for a subset of them is not defined; the refusal is on the same property that
makes \code{adaptive} refuse a preconditioner, and it is raised when the mixer is
built rather than at the first mix, so a run that cannot work does not find out
hours in. A non-positive value is refused for the same reason any step length
would be.

It acts on the \textbf{density} alone. \code{becsum}, \code{ns} and $\tau$ keep
the charge's step, because giving them the magnetic one would be a second
departure with no measurement behind it. \code{becsum}'s own residual is now
reported every iteration --- outside \code{dr2}, where it cannot be, since
\code{paw\_ddot} is not positive definite and \code{pw.x} comments the term out
for that reason --- so whether it needs a step of its own can be asked with a
number.
\end{refuses}
...
\code{pw.x}. A channel the constraint leaves with no electrons runs, as it does in
\code{pw.x}: it is filled with nothing and reports \code{iweights}' own $-10^{20}$
as its level, and the fully polarized hydrogen atom at
\code{tot\_magnetization = 1} lands on \code{pw.x}'s $-0.94606495$~Ry to every
printed digit. A $|$\code{tot\_magnetization}$|$ larger than the electron count
is refused, where \code{pw.x} fills the other channel anyway and stops on the
wrong charge.
...
\textbf{A field lowers the symmetry group, and here that includes a uniform
one.} A uniform \code{B\_field} is an axial vector on every atom, so in a
noncollinear run it enters the magnetic symmetry filter beside the moments, as
Elk's \code{findsym} does with \code{bfieldc0}; \code{pw.x} hands its filter
the moments alone, so a \code{pw.x} run with a field off the moment's axis keeps
the larger group. On bcc Fe along $z$ with \code{B\_field(1) = 0.01} this drops
the $C_{4z}$ that maps $B_x$ onto $B_y$, which is what had been averaging the
canting away. A \code{LOCAL\_MAGNETIC\_FIELDS} card labels its atoms in the
same way at \code{nspin = 2}, so a staggered field on two otherwise equivalent
atoms keeps only the operations that do not swap them. To change the field of
an existing system, use \code{system.with\_b\_field(...)}, which rebuilds the
k-set with the new group.

\textbf{A field needs no starting moment, on a cell that is not a magnet
already.} A noncollinear run with a \code{B\_field} and every
\code{starting\_magnetization} zero is magnetic because of the field, and a
gradient-corrected functional takes the sign of the magnetization along the
field. Where the field-free ground state is nonmagnetic this converges to the
induced moment: a hydrogen atom smeared at \code{degauss = 0.2}, whose seeded
field-free run decays to $10^{-4}\,\mu_B$, takes $+0.159\,\mu_B$ along a
\code{B\_field(3) = 0.01}. Where the cell polarizes on its own it does not: the
same atom at \code{degauss = 0.02} converges to $-0.103\,\mu_B$, against the
field, which is the stationary point near zero moment that an SCF finds as
readily as a minimum, 0.052~Ry above the polarized state. A spontaneous magnet
still wants a seed of the size of the moment it will carry, and the field does
not choose its sign: a seed of 0.6 along the field reaches $+1.0\,\mu_B$, a seed
of 0.05 crosses to $-1.0\,\mu_B$ at the same total energy, the field's energy
being outside the total. This departs from \code{pw.x}, whose \code{domag} reads
the starting moments alone: it accepts the unseeded input and converges the
\emph{field-free} calculation, because \code{add\_bfield} writes the field into
the potential's magnetization channels and \code{vloc\_psi\_nc} applies those
only \code{IF (domag)}. A comparison against \code{pw.x} for such an input is
therefore a comparison against no field.

An \code{r\_m} left unset for a species, or set to
zero, takes \code{make\_pointlists.f90}'s default radius rather than a sphere of
radius zero.
...
\textbf{Also refused, as \code{pw.x} refuses them:} \code{'total direction'}
at \code{nspin = 2}, since the total moment of a collinear run has one component
and its polar angle is $0$ or $180^\circ$, meaning that the penalty is a
constant with no gradient (\code{add\_bfield.f90:177-180}); and
\code{tot\_magnetization} at any \code{nspin} other than 2, which is
\code{pw.x}'s own ``tot\_magnetization requires nspin=2''
(\code{input.f90:778-782}). Both used to run: the first reported a constant
constraint energy over an unconstrained state, and the second was stored and
read by nothing, so an unpolarized or noncollinear run converged with a moment
different from the one asked for. A noncollinear run holds a total moment with
\code{constrained\_magnetization = 'total'} instead.
\end{refuses}
...
\textbf{On a robust magnet it holds a texture exactly, and it needs Elk's
reading, Elk's mixer and Elk's gain to do it.} Two iron moments at 90 degrees
without spin-orbit coupling (\code{tests/data/qe/fe2-canted-nosoc.in}, bcc iron
in its conventional cell), where exchange wants the pair parallel and holding it
costs a real torque:
%
\begin{verbatim}
from defumat import Calculator
text = open("tests/data/qe/fe2-canted-nosoc.in").read().replace(
    "    nosym = .true.\n",
    "    nosym = .true.\n    constrained_magnetization = 'atomic fsm'\n"
    "    lambda = 0.02\n")
text += "STARTING_MOMENTS\n  1.868 0 0\n  0 1.868 0\n"
calc = Calculator.from_text(text, "tests/data/pseudo")
scf = calc.get_scf(mixing_mode="adaptive", mixing_beta=0.05)
scf.site_moments          # (1.868, 0, 0) and (0, 1.868, 0)
scf.magnetic_field.atomic # the field that holds them, 0.026 Ry per atom
\end{verbatim}
%
It converges in 46 iterations with the pair at $90.002^\circ$ and both lengths on
the target to $10^{-4}\,\mu_B$. The vector penalty on the same cell holds
$89.2^\circ$ ($0.39^\circ$ per site) in 76 at $\lambda = 1$, the largest it
tolerates, and Elk itself holds the direction-only variant in 55 loops under
$0.0097$~Ha per atom, which is $0.019$~Ry in this code's units. What a penalty
cannot give and this does is the field: $\mathbf B_i$ is the torque the texture
costs, perpendicular to each moment to within a quarter, and reading it is
reading how far the state is from being stationary on its own.

Three choices make it work and each was measured to be necessary. The moment the
update reads is the iteration's \emph{output} density, as Elk reads its
\code{mommt}; reading the mixed one instead lets the field climb to $0.25$~Ry
while the moment it is turning does not move. The mixer carries no history,
\code{mixing\_mode = 'adaptive'} at \code{mixing\_beta = 0.05}, which is Elk's
\code{mixadapt} at Elk's \code{beta0} and is what Elk ran: the field changes the
calculation's fixed point every iteration, and Anderson extrapolates over maps
that no longer exist, which on this cell runs away at every gain from $0.0025$ to
$0.04$. And the gain is Elk's, below.

The earlier verdict on this scheme, measured on the two-hydrogen cell below, was
that it does not converge: a ring with a growing envelope, and a secant that
converged to the wrong state. That cell carries $0.000235\,\mu_B$ unconstrained,
so its $m(B)$ is nearly a step and it is the wrong cell for any controller; what
was wrong with the scheme was the first two choices above. The \code{secant}
update steps only when the inner calculation has converged, and on the iron pair
that gate is its failure: under a held field the inner run plateaus at
$5\times10^{-5}$, the field freezes, and 800 iterations end where 200 did. The
default update is therefore \code{'elk'}.
...
give a field in Rydberg. Elk's own default is 0.01 in \emph{Hartree} per Bohr
magneton, added to the Kohn-Sham field directly, which is \textbf{0.02} here, and
0.02 is the default: at 0.01 the iron pair is still moving at $0.8^\circ$ per site
after 300 iterations, and at 0.02 and 0.04 it converges in 63 and 64. Write a
penalty-sized value and the run warns by name: at $\tau = 0.2$ the vector scheme
on the hydrogen cell drives the moments to $0.82\,\mu_B$ against a target of
$0.26$.
...
\textbf{Warned about, with a number:} a per-atom feedback scheme
(\code{'atomic fsm'}, \code{'atomic fsm direction'}) under the \code{'elk'}
update with any mixer but \code{'adaptive'}. The run is legal and will not
converge: on the canted iron pair Anderson runs away at every gain tried, where
the adaptive mixer holds the pair to $0.001^\circ$. The warning is raised before
the first iteration and names the mixer and \code{beta0} to use.
...
\textbf{A symmetry-reduced k-set works}, and what makes it work is that, in
a magnetic noncollinear run, $\langle L\rangle$ and $\langle S\rangle$ are
averaged over the group as \textbf{axial} vectors --- each operation carrying $\det(R)$ and the
...
number. \textbf{A collinear run is different}, because there the spin is not
coupled to the lattice: at \code{nspin = 2} the site $\langle S_z\rangle$ is a
scalar carried from atom to atom by the operations, and averaging it as an axial
vector under the spatial group gave exactly zero on bcc Fe's 48 operations.
$\langle L\rangle$ of a collinear run is completed to zero by time reversal,
since each channel has a real Hamiltonian and $\langle L\rangle_{-\mathbf k} =
-\langle L\rangle_{\mathbf k}$. A \textbf{nonmagnetic spinor run} is completed to zero in
the same way, and for both vectors: with no magnetization the ground state is
time-reversal symmetric, so every state at $\mathbf k$ has a Kramers partner at
$-\mathbf k$ with $\langle L\rangle$ and $\langle S\rangle$ reversed, and on a
wedge cut with $\mathbf k \to -\mathbf k$ both are set to zero at every site.
The spatial average alone does not do it on a noncentrosymmetric cell, or at
$n_{\rm sym} = 1$, where it returned the uncancelled half of the zone.
...
\textbf{The cheaper thing gives almost none of it.} Freezing the wavefunctions
as well and taking $\langle\psi|H_{\text{SOC}}|\psi\rangle$ once, at the
coupling-free states, gives the first-order term, and the coupling enters there
as $\xi\,\langle \mathbf L\rangle\cdot\mathbf n$, which vanishes because the
orbital moment of a scalar-relativistic collinear state is quenched. What
survives is the exchange field's augmentation charge seen through the
$j$-resolved projectors, the spin times a time-even orbital tensor, which a
quenched state does not remove: on tetragonal cobalt it is $1.79\times10^{-3}$~meV
of anisotropy with the easy axis along $c$, against $0.552$~meV of free-energy
anisotropy from the diagonalisation on the same density, and on a cubic cell it
is the same $1.26\times10^{-2}$~meV along every axis. So the anisotropy is
second order in the coupling to about $0.3$ per cent, and what supplies it is
the repulsion between levels that a diagonalisation performs and an expectation
value does not. \code{get\_first\_order\_soc(spinor, direction=)} computes the
first-order term, $\sum w\,\langle\psi|\delta D - \varepsilon\,\delta
S|\psi\rangle$ with $\delta D$ and $\delta S$ the coupled nonlocal coefficients
and overlap minus the reduced ones at the same potential, which is the
derivative of the free energy with respect to \code{soc\_scale} at $0$.
...
\code{soc\_scale} scales the spin-orbit part of the nonlocal potential, the
overlap and the augmentation charge while keeping the same dataset and the same
k-points. At $0$ every one of them is replaced by the spin trace of its
spin-orbit form, $\tfrac12\sum_{s s'} F^{s s'} M F^{s' s}$ for each
spin-independent matrix $M$ the coefficients $F$ dress, the density's
\code{becsum} included, so the reduced Hamiltonian is the derivative of the
energy it reports. At $0$ the anisotropy is exactly zero, which is the control
for the whole calculation: without the coupling the Hamiltonian is invariant
under a global spin rotation, so a band energy \emph{cannot} depend on where the
moment points. On tetragonal cobalt the two directions of a self-consistent
total agree to $2\times10^{-10}$~meV, and switching the coupling off costs
$0.44$~mRy of the total. On a fully-relativistic PAW dataset it also scales the
magnetization the Dirac small component carries on each sphere,
$-2(\mathbf m_{\text{s}}\cdot\hat{\mathbf r})\,\hat{\mathbf r}$, which is the
spin reflected about the radial direction and so ties the spin to the lattice as
the coupling does; its charge is left alone, being invariant under a spin
rotation. Left on at $0$ it gave tetragonal nickel
($\texttt{Ni.rel-pbe-spn-kjpaw\_psl}$, $40$~Ry, a $2\times2\times2$ mesh)
$8.3\times10^{-3}$~meV between $x$ and $z$; switched off with the rest, the two
agree to $7\times10^{-6}$~meV at \code{conv\_thr = 1e-12}, a residue that
does not fall further with \code{conv\_thr} and belongs to the truncated cutoff:
at the dataset's own $75/480$~Ry the two agree to $3.9\times10^{-9}$~meV.
...
\textbf{The torque as a vector: every spin turned by one rotation.} The angle
above turns a collinear moment in one plane. \code{get\_orientation\_torque}
turns the whole texture by a rotation $R$, $\mathbf m(\mathbf r)\to
R\,\mathbf m(\mathbf r)$ with the charge kept, diagonalises once with the
coupling at that orientation, and returns
\begin{equation*}
  \boldsymbol\tau = -\frac{\partial F}{\partial\boldsymbol\omega}\bigg|_{\boldsymbol\omega=0},
  \qquad R(\boldsymbol\omega) = e^{[\boldsymbol\omega]_\times} R_0 ,
\end{equation*}
the torque for the three generators of a rigid rotation about $R_0$, in Ry per
radian, meaning that component $a$ is how fast the free energy falls when every
spin turns together about $\hat{\mathbf e}_a$. For a collinear magnet turning in
the plane $(\hat{\mathbf e}_1,\hat{\mathbf e}_2)$ the plane torque above is
$\boldsymbol\tau\cdot(\hat{\mathbf e}_1\times\hat{\mathbf e}_2)$, and the
component about the moment is zero, since that turn moves nothing. On the
tetragonal cobalt pair of the table, 45 degrees from $c$ towards $a$, the $y$
component is the plane torque to $10^{-10}$ relative on the same states,
$-4.0595\times10^{-5}$~Ry, and the component that tilts the moment out of the
$(z,x)$ plane, which the mirror $y\to-y$ with time reversal forces to zero, is
$5.8\times10^{-11}$~Ry, the diagonalisation's level.

\begin{pycode}
import numpy as np
from defumat.workflows.anisotropy import rotation_from_euler
R = rotation_from_euler(0.0, np.pi / 4, 0.0)     # ZYZ, radians: c tilted 45 deg to a
vector = scalar.get_orientation_torque(spinor, rotation=R)
print(vector.torque)                             # -> [~0, -4.06e-05, ~0] Ry/rad
print(vector.direction, vector.along_moment)     # where the moment points; ~0
\end{pycode}

\noindent The orientation is given as a matrix and read back as ZYZ Euler angles
(\code{vector.euler\_angles}), but nothing is differentiated in the angles: at
$\beta=0$ the angles $\alpha$ and $\gamma$ are the same rotation, and that is an
easy axis along $z$. The source may be a four-component density whose moments
are not collinear, a commensurate spiral in a supercell for instance, and it is
then turned as it is, with the system's \code{angle1}/\code{angle2} describing it.
Two checks that share no automatic differentiation with the torque: the closed
form
\begin{equation*}
  \boldsymbol\tau = \int \mathbf m_{\rm out}(\mathbf r)\times\mathbf B(\mathbf r)\,\mathrm d\mathbf r ,
\end{equation*}
the frozen exchange field acting on the magnetization of the states solved with
the coupling, which agrees with it to $1.7\times10^{-12}$ on ultrasoft cobalt;
and turning the lattice by $R^{-1}$ under fixed spins in place of turning the
spins by $R$, which agrees to $4\times10^{-11}$ once the eigensolver's floor is
lowered, and to $10^{-6}$ at the default floor, which is the torque's
resolution there (about $10^{-10}$~Ry per radian).

\textbf{Relaxing the orientation.} \code{get\_relaxed\_orientation} is the
structural relaxation with the rotation as the coordinate: at each step one
diagonalisation with the coupling at the frozen source density turned rigidly,
the free energy and the torque above, and a step of the same BFGS the atoms
use, in the rotation vector about the start, so a length is an angle in
radians. It returns the easy axis of a magnet, or the plane (and phase) of a
spiral in a supercell.

\begin{pycode}
start = rotation_from_euler(0.4, 0.9, -0.3)      # 51.6 degrees off c, oblique
relaxed = scalar.get_relaxed_orientation(spinor, rotation=start, curvature=True)
print(relaxed.converged, relaxed.direction)      # -> True, (~0, ~0, 1): the c axis
print(relaxed.curvature_eigenvalues)             # -> [~0, 2K1, 2K1] Ry/rad^2
\end{pycode}

\noindent Start it off every symmetry element: an orientation a symmetry fixes
has no torque, whether it is an easy axis or a hard one, and a relaxation
started there reports convergence without having searched (it warns). The
curvature, a central difference of the torque, is what tells a minimum from a
saddle; one of its eigenvalues is zero for a collinear texture, the turn about
the moment itself. The gradient threshold sits above the torque's noise, and
the energy minimised is the free energy $\sum w\varepsilon - TS$, for the reason
above.

\textbf{The same inside a self-consistent run with the coupling.} The
orientation is an electronic degree of freedom, so an SCF with spin-orbit
coupling turns the moments itself, but it is a soft mode: the lean of the output
magnetization off the field it was solved in is the anisotropy over the
exchange, and a mixer takes a fraction of it per iteration. Seeded 45 degrees off
$c$, tetragonal cobalt does not converge in 100 iterations, its angle wandering
between 36 and 54 degrees while $\mathrm{d}r^2$ falls to $10^{-8}$ and rises
again. Every iteration has done the force theorem's calculation with the input
density frozen, so the torque on a rigid turn of that density is the closed form
above, one integral per iteration, recorded as
\code{SCFResult.orientation\_torque} and in \code{history}; it sits at
$-3.3\times10^{-5}$~Ry per radian through that whole run, which is $-K_1$ of the
self-consistent anisotropy read from one run. \code{rotate\_moments=True} turns
the input density by $\boldsymbol\omega=-H^{-1}\mathbf G$ after every mix, the
density, \code{becsum}, the mixer's history and the carried states together,
with $H$ built by BFGS from the torques and every step bounded by
\code{rotation\_trust}, and the run is converged when the torque is below
\code{torque\_conv\_thr} as well.

\begin{pycode}
oblique = Calculator.from_file("oblique.in")     # lspinorb, nosym, angle1 = 45
scf = oblique.get_scf(rotate_moments=True, conv_thr=1e-12, torque_conv_thr=1e-9)
print(scf.converged, scf.magnetization_vector)   # -> True, along c
print(scf.orientation_torque)                    # -> ~1e-9 Ry/rad
\end{pycode}

\noindent It converges in 44 iterations to 0.002 degrees from $c$, and its total
energy is that of the SCF started exactly along $c$ to $1.3\times10^{-8}$~Ry.
Without the coupling every orientation has the same energy and the torque is the
eigensolver's noise, which a step divided by a vanishing curvature would turn
into motion, so the option is refused there rather than left to do nothing.

\begin{refuses}
\textbf{Refuses}, for \code{rotate\_moments}: no coupling (or \code{soc\_scale}
$=0$); a spin spiral, whose transverse magnetization is kept in the frame that
turns with $\mathbf q$; a Hubbard $U$, whose \code{ns} would have to turn with the
density; a magnetic field or a constrained moment, which holds the orientation
the step moves; and a run without \code{nosym}. \code{get\_relaxed\_orientation}
and \code{get\_orientation\_torque} refuse everything the force theorem refuses.
\textbf{PAW is taken by both}: the one-centre coefficients are rebuilt from the
turned \code{becsum} inside the differentiated energy, from the one-file route's
\code{soc\_scale} $=0$ leg for the force-theorem routes, and on nickel the
one-centre share of the torque is larger than the whole and of the opposite
sign. With the coupling, \code{becsum} is not a vector under a spin rotation
(it keeps only the blocks diagonal in $j$), so \code{rotate\_moments} turns it
through the states. On PAW nickel \code{rotate\_moments} still stalls near
$10^{-4}$, its steps running along a nearly flat in-plane direction; prefer
\code{get\_relaxed\_orientation} there.
\end{refuses}
...
sampled on two different wedges. A four-component source density that is
genuinely noncollinear is refused by name, as it already was on the path that
builds the states: the torque lays the moment down along one axis with its
sign, and an antiferromagnet keeps its antiparallel regions only because of
that sign. \code{soc\_scale} takes $0$ or $1$
...
$E(\mathbf q) - E(0) = m^2 \sum_{s} J_s \sum_{\mathbf R \in s}[1 - \cos(\mathbf q\cdot\mathbf R)]$,
one $J_s$ per neighbour shell $s$, which is how a spiral scan becomes a spin
model. Each shell is named by one representative lattice vector, and the inner
sum runs over its whole star under the lattice point group (or the
\code{point\_group} passed in), each $(\mathbf R, -\mathbf R)$ bond counted
once, so the fitted $J_s$ does not depend on which member of the star was
typed.
...
the one being differentiated. \code{heisenberg\_exchange} refuses a shell
vector that is not a nonzero integer lattice vector in crystal coordinates, and
two representatives of the same star, whose two $J$ would be one parameter and
leave the fit singular.
...
above the four moments come back at $28^\circ$, $66^\circ$, $-42^\circ$ and
$-51^\circ$ in the plane the field turns in, with nothing out of that plane
beyond $2\times10^{-6}$ (on the reference's states alone, $27^\circ$, $62^\circ$,
$-40^\circ$ and $-48^\circ$, $0.30$~mRy higher in \code{E + field}), so they
follow the field, but by less than the $90^\circ$ per cell it asks for.
...
On the four-cell hydrogen ultracell above, on the reference's states alone,
every run given 300 iterations: at a $0.01$~Ry turning field every \code{beta}
converges to the same state and the best is $0.5$ (36 iterations, against 48 at
$0.7$ and 115 at $0.2$). At
...
when it fails to converge. \textbf{The default basis makes the manifold exactly
flat}, since it prefers no direction where the reference's states alone prefer
their own: the $0.01$~Ry run at \code{beta = 0.3} takes 78 iterations on it
against 53 on the old basis, to a state $0.30$~mRy lower.
...
the reference's $0.0743$. That ladder is the reference's states alone. On the
default basis there is none: without spin-orbit coupling the closed span holds
the rotated states exactly, and the moment is right to $7.5\times10^{-7}$ at
\code{nbnd = 16} with \code{E + field} equal to twelve digits. Reading that
needs the reference converged in \emph{direction}, which a flat manifold makes
slow: at \code{conv\_thr = 1e-11} it stops $2.5\times10^{-2}$ degrees short of
$\hat{\mathbf B}$, and at \code{1e-13} it is within $5.6\times10^{-5}$.

\textbf{On the reference's states alone, expect to need more bands than a
collinear run, by about a factor of two.} A spinor state holds one electron and lives in a space of
...
mind. This paragraph and the three after it describe the reference's states
alone, \code{kramers\_pairs=False}; the default basis, closed under time
reversal and described after them, removes the preferred axis, the canting and
the lean they measure. Such a helix is invariant under a translation by one cell followed by a
...
teaches, arriving from the other side. A reference in the helix plane is
worse: the same seed then converges in 23 iterations to steps of 94, 74 and 84
degrees with a uniform moment of 0.069 along the reference, which is a wrong
answer rather than a slow one. \textbf{The run says so}: a seed whose
directions do not sit on a cone about the reference's magnetization is warned
about by name, and the warning names the fix below.
...
\textbf{The fix for the frame, the cone and the lean is a basis closed under
time reversal.} All three are the basis preferring the reference's own
direction: its majority orbitals carry spin along $+\hat{\mathbf e}_0$ only, so
turning a moment away needs a majority orbital with the other spinor, which the
basis does not have. \code{kramers\_pairs}, \textbf{on by default for a
magnetized noncollinear reference}, adds each frozen state's Kramers partner,
the states of the reference with its magnetization reversed, from a second
fixed-density solve, and diagonalises the reference Hamiltonian in the union
once, before the loop; \code{kramers\_pairs=False} asks for the reference's
states alone, which is what every noncollinear number before it was. Without spin-orbit coupling that span is the
orbitals times both spinors, which prefers no direction at all. On the same four
cells of hydrogen, at the same 16 states per folded k-point:

\begin{center}
\begin{tabular}{llllll}
basis & reference & steps & cone & uniform moment & $E$ above the supercell \\
\hline
\code{nbnd = 16} & along $z$ & $90.0001^\circ$ & $10.64^\circ$ & $0.0628$ & $+4.37\times10^{-4}$ \\
\code{nbnd = 16} & along $x$ & $94^\circ$, $74^\circ$, $84^\circ$ & $0$ & $0.069$ & $+4.99\times10^{-4}$ \\
Kramers, \code{nbnd = 8} & along $z$ & $90.0000^\circ$ & $0$ & $0$ & $+1.3\times10^{-6}$ \\
Kramers, \code{nbnd = 8} & along $x$ & $90.0000^\circ$ & $0$ & $0$ & $+1.3\times10^{-6}$ \\
\end{tabular}
\end{center}

in Ry per cell, 10 iterations for both closed runs against 14 and 23, and at
twice the size the old basis reaches only $3.81^\circ$ and $+1.56\times10^{-4}$.
The two sets are nearly dependent, since a band with little exchange splitting
has nearly the same orbital in both spin channels: directions whose overlap
eigenvalue falls below $10^{-8}$ are dropped, and \code{kramers\_ranks} says how
many each folded k-point kept. Those small directions are the ones that matter,
the differences between the majority and the minority orbitals, and a floor of
$10^{-2}$ keeps the closure and loses the accuracy ($+1.96\times10^{-4}$). It
costs a second frozen solve and a basis of up to twice \code{nbnd}, so four times
the matrix, so \code{nbnd} buys twice the states it does without it. With
spin-orbit coupling the partners remove only the preference
between $+\hat{\mathbf e}_0$ and $-\hat{\mathbf e}_0$, and on NiBr2 that is
enough: a three-cell PAW helix at \code{nbnd = 40} keeps a uniform moment of
$1.3\times10^{-4}$ of its length in both frames, the order of Elk's
$9\times10^{-5}$, where the old basis keeps $0.415$ with the reference along the
axis and $0.300$ in the plane, and still $0.288$ and $0.188$ at twice the bands; the two frames agree to $10^{-8}$~Ry and lie
$2.2$~mRy below the old basis at the same size.
...
ulr = cal.get_ultracell(supercell=(4, 1, 1), kgrid=(1, 2, 2), nbnd=24,
                        ...
                        kramers_pairs=False)   # the reference's states alone
...
# the default, a basis closed under time reversal, at a third of the size
closed = cal.get_ultracell(supercell=(4, 1, 1), kgrid=(1, 2, 2), nbnd=8,
                           seed_magnetization=helix, mixing_beta=0.3)
m = closed.cell_moments()
print(closed.iterations)                           # 11
print(np.degrees(np.diff(np.unwrap(np.arctan2(m[:, 1], m[:, 0])))))  # 90 90 90
print(m.mean(axis=0))                              # [0, 0, 0] to 1e-7: no remnant
print(closed.total_energy - ulr.total_energy)      # -2.52e-4 Ry per cell
print(closed.kramers_ranks)                        # 16 at every folded k-point
\end{pycode}
...
\begin{refuses}
\code{kramers\_pairs=True} on a collinear cell, whose one channel's partner is
the other channel and is already in the basis, and on a nonmagnetic spinor
cell, whose states already come in Kramers pairs.
A reference whose k-set is not the unshifted, unreduced
$\code{supercell} \times \code{kgrid}$ Monkhorst-Pack grid. The tiled unit-cell
density is a fixed point of the loop only when the unit cell integrated the zone
over exactly the points the ultracell folds onto, so on any other set the loop
converges to a different sampling and reports the difference as a modulation:
0.635~Ry per cell with nothing applied, on silicon converged at \code{4 2 2} and
folded onto \code{2 1 1}. The check is on the points, so an explicit list that
is that grid passes, and a symmetry-reduced wedge is refused by name. Left out,
\code{kgrid} in \code{get\_ultracell} is the reference's grid divided by
\code{supercell}, which is the one value that passes. Occupations other than
\code{'fixed'} and \code{'smearing'}: a tetrahedron weight interpolates a band
across a mesh, and an ultracell state at one $\mathbf k_0$ is a mixture across
the $N$ folded points, so it is not a band of any mesh (Elk's
\code{occupyulr} has only a smeared branch), and \code{'from\_input'} has no
band left to hold its prescribed occupation. Both used to reach the smeared
branch at \code{degauss = 0} and return NaN.
