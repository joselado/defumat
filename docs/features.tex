...
\begin{refuses}
\textbf{Refuses.} \textbf{Ultrasoft and PAW}, and what is missing is now the
\emph{assembly} rather than the term. The term --- the augmentation dipole
$\langle\psi_n|T^\dagger\partial_k T|\psi_m\rangle$, which belongs beside the
$\epsilon_n\,\partial S/\partial k$ this assembly already has --- is written
and validated as \code{topology.kubo.augmentation\_connection}. What this
module cannot yet do with it is hold the two slots apart: it contracts
$z = \langle n|v_i|m\rangle\langle m|v_j|n\rangle$ out of one array, and the
corrected factors are not transposes of each other, the first taking
$K^\dagger$ and the second $K$. The Drude term is unaffected, since the
correction carries $\varepsilon_n - \varepsilon_m$ and vanishes on the
diagonal. For the static limit alone, \code{topology.kubo} does carry it. A
...
one order further out, and one of the two terms is now written. The dipole
carries $\partial S/\partial k$ and its derivative carries
$\partial^2 S/\partial k_a\partial k_b$, and both are here; the augmentation
dipole $\langle\psi_n|T^\dagger\partial_k T|\psi_m\rangle$ that belongs with
them is here too, as \code{topology.kubo.augmentation\_connection}. What is
still missing is its own derivative, $\partial_k(T^\dagger\partial_k T)$ ---
a second projector derivative against $\mathrm{dpqq}$ and $\mathrm{dpqq}$'s own
first moment --- which this module needs because it is that matrix element one
order further along. A
