...
in the order of \code{ATOMIC\_POSITIONS}. QE's \code{starting\_magnetization},
...
\textbf{One thing it does not yet reach, for a PAW dataset:} the atomic
\code{becsum}, which is still split by the per-\emph{species}
\code{starting\_magnetization}. So on PAW the charge's starting texture and the
one-centre starting occupations disagree at iteration~1. The SCF repairs it and
nothing downstream is wrong at convergence; it costs iterations, and it is
written down here rather than left to be met.

\textbf{Not covered, and this one is a sentence rather than a \code{raise}:} a
textured starting \emph{density} handed to \code{run\_scf} by the caller. The symmetry group is decided from the input and
never from the density, so a texture that reaches the run only that way is
invisible to the filter and will be symmetrised away. Give the same texture as a
\code{STARTING\_MOMENTS} card, or set \code{nosym = .true.}
\end{refuses}
