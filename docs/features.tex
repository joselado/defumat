...
sts.integral      # (41,) D(E) per unit cell: the sum rule
...
axis. Pass \code{grid} (which builds the \emph{complete} grid here, where an
image's reduces) or run \code{nosym}. A tip inside an \textbf{augmentation
sphere} of an ultrasoft or PAW dataset, where the pseudo-wavefunction is not the
true one and the norm is short by 3 to 9 per cent. An ultracell is exempt from
