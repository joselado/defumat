...
off-diagonal blocks are what let the shell's moment point anywhere. Relativistic
BN against \code{pw.x}: \textbf{1.2e-7 Ry}, with $\mathrm{Tr}[n^{\sigma\sigma}]$
matching every digit \code{write\_ns} prints.
...
pointing somewhere else. The \textbf{full} functional on a spinor is refused
too, and that one is measured rather than assumed: each axis agrees with
\code{pw.x} on its own (1.2e-7~Ry for a spinor, 4.2e-9 for the full functional
on a collinear cell) and together they sit 4.2e-4~Ry apart with both codes
converged and on the same solution, so a term is missing.
\code{hubbard\_slater = 'yukawa'} needs a \textbf{PAW}
...
% =====================================================================
\section{Effective masses, site angular momenta, nesting and structure factors}

Quantities Elk computes and \code{pw.x} does not. The first two cost one
