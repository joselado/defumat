...
determines) and a \emph{Hubbard setup} (without it \code{ns} is an array about
nothing); an applied field is saved normally. \code{stress}, \code{solver} and
...
\begin{refuses}
\textbf{A mixer whose steps grow can change which solution the run finds}, and
this is the thing to know before trusting an \code{adaptive} result on a magnet.
On the four-cell hydrogen ultracell under a turning field --- the case where
rotating every moment together costs nothing --- it converges in 165 to 172
iterations against Anderson's 265, and to a \emph{different state}: the per-cell
moment comes out at $0.186\,\mu_B/2$ against Anderson's $0.978$, a factor of five
in the \emph{length} of the moment rather than a rotation of it. It is the
\emph{mixer} and not the step size: Anderson was run at $\beta = 0.1$, $0.3$ and
$0.7$, a factor of seven, and all three sit between $0.98$ and $1.00$, while
\code{adaptive} at two values of $\beta_0$ a factor of four apart sits at $0.186$
twice. \textbf{Which of the two is the ground state has not been established} ---
neither matches the $0.62\,\mu_B/2$ the record gives for this system's unit cell
--- so this is a warning that the two disagree, not a claim that the adaptive one
is wrong. Two values of
$\beta_0$ a factor of four apart agree with each other to $6\times10^{-5}$, so it
is systematic and not a stray trajectory, and both runs report
\code{converged = True} with an \code{accuracy} three orders below
\code{conv\_thr}. Nothing in the result says which state is the right one.

So on a cell with a soft or degenerate magnetic direction, run it against
\code{anderson} and compare the \textbf{moments}, not just the energy: a
converged run is not evidence that the basin was kept. \code{SCFResult.site\_moments}
is what to compare.
...
noncollinear branch --- run with \code{nosym} and the whole $\mathbf k$-grid,
where both codes use every point and neither averages, or pass
\code{symmetrize=False}. \emph{The two branches are missing different things,
and only one of them is missing a matrix.} Without spin--orbit coupling the
columns are $|l m\rangle \otimes |\sigma\rangle$ and the operator is the tensor
product $D^l \otimes S$, both factors of which now exist (the harmonic rotations
and the $SU(2)$ ones); what is left there is the coefficient table, which is
real over $2l+1$ columns and would have to be complex over $2(2l+1)$. With
\code{lspinorb} the columns are $|j\, m_j\rangle$ and the operator is $D^j$ for
$j = \tfrac12 \ldots \tfrac72$, which is a different matrix and is not that
tensor product. A \textbf{fully-relativistic dataset with
...
without saying so. A \emph{Hubbard setup} is refused too. \code{checkpoint\_every}
must be at least 1.
...
\textbf{A spinor run has a dielectric constant and Born charges}, on a
norm-conserving dataset. A nonmagnetic one is the same physics on a doubled
...
that cell's 157464 grid points, so it is not a convention at an edge. It is
unlocated, and a number that is 5\% out while looking like a working calculation
...
dielectric constant and Born charges on a \textbf{norm-conserving} dataset,
provided it carries \emph{no net moment}. A \textbf{textured} one
...
\emph{ultrasoft or PAW} one is refused by name and the missing piece is exactly
one object: $\mathrm dD_{ij}$ is a $2\times2$ matrix in spin space
(\code{set\_int3\_nc}) where a norm-conserving dataset has no $\mathrm dD$ at
all. The \textbf{phonons, the Raman tensor, the strain response} and everything
...
nothing else. The convergence carries no sign --- the Hamiltonian moves with
its own truncated density, so an ultracell eigenvalue is not an upper bound on
the supercell's --- but it does converge, which is the property the
implementation is checked against: a two-cell silicon ultracell under an
applied modulation reproduces a real four-atom supercell's induced density, and
the disagreement falls monotonically with \code{nbnd}
(\code{tests/regression/test\_ultracell.py}).
...
Nothing in a collinear calculation breaks spin symmetry on its own, so a
modulated moment has to be driven. \code{magnetic\_field} is the knob: a
collinear $B(\mathbf r)$ in Ry, entering as $v_\uparrow \mathrel{-}= B$ and
$v_\downarrow \mathrel{+}= B$, in the same two forms \code{external} takes.
...
\textbf{There is no total energy}, in this code or in Elk, whose
\code{energyulr.f90} reports the sum of occupied eigenvalues and the Fermi level
and nothing else. \code{ulr.band\_energy} is that sum, per unit cell. The energy
\emph{gain} of a modulated state over the uniform one --- the quantity that says
whether a spin density wave is the ground state at all --- is therefore a
quantity neither code computes.

\begin{refuses}
Ultrasoft and PAW datasets: the augmentation charge is a function of the density
through $D_{ij}$, so the frozen unit-cell states stop being a fixed basis as soon
as the modulation moves. Gradient-corrected functionals: the gradient of the
...
Meta-GGA. An applied \code{magnetic\_field} on a noncollinear cell that
...
$\Gamma$. A double grid, the wavefunctions and the density then living on
different ultracell boxes. Symmetry is not used at all: a modulation breaks the
...
exactly, with no factor between them, and does so to $10^{-11}$; that identity
is what holds the two normalisations against each other, since the image carries
its $N$ in the weights and the transmission carries it in the volume the states
are normalised over.
...
a tip sees one energy. An applied \code{magnetic\_field} is \textbf{not} refused
